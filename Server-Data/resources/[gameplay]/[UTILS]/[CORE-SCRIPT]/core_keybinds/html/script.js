...
function closeMenu() {
    $.post('https://core_keybinds/close', JSON.stringify({}));
    ...
    if (keybind) {
      ...
      $( "div" ).each(function() {
          ...
          if (key != "-" && command != "none") {
               ...
               $.post('https://core_keybinds/registerKeybind', JSON.stringify({command: command, key: key}));
          }
    ...
    $.post('https://core_keybinds/updateKeybinds', JSON.stringify({}));
...
'   <div class="clearfix grpelem" id="u1545"><!-- group -->';
 ...
 base = base + '<div id="keybind-select">' +
'     <div class="gradient colelem keybind " onclick="keybindFocus(this)" id="u1557" >'+key+'</div>' +
'<select  id="u1558">' +
'    <option value="none">-</option>';

for (g = 0; g < Object.keys(options).length; g++) {
  if (options[g].command == value ) { 
    base = base + '    <option class="select-items" selected="selected" value="'+options[g].command+'">'+options[g].label+'</option>';
  } else {
    base = base + '    <option class="select-items" value="'+options[g].command+'">'+options[g].label+'</option>';
  }
...
for (b = 0; b < Object.keys(options).length; b++) {
    ...
    base = base + '    <option value="'+options[b].command+'">'+options[b].label+'</option>';
...
function clickBox(t) {
    ...
    $.post('https://core_keybinds/' + trigger, JSON.stringify({}));
...
'   <div class="clearfix grpelem" id="u1541"><!-- group -->';
  for (i = 0; i < Object.keys(boxes).length; i++) {
    ...
    if (boxes[i].jobWhitelist.includes(job) || Object.keys(boxes[i].jobWhitelist).length == 0 ) {
          base = base + '     <div class="gradient colelem box " id="u1556" data-trigger="'+boxes[i].trigger+'" onclick="clickBox(this)"><i class="'+boxes[i].icon+'"></i><p class="label">'+boxes[i].label+'</p></div>';
...
if (fill) {
  ...
  base = base + '     <div class="gradient colelem box" id="u1556"></div>';
}
}
