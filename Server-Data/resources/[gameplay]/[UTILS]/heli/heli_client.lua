...
AddEventHandler('heli:spotlight', function(serverID, state)
	...
	SetVehicleSearchlight(heli, state, true)
...
function IsPlayerInPolmav()
	...
	local polmav2 	= GetHashKey("as332")
	local polmav3 	= GetHashKey("sheriffmav")

	if polmav then
		return IsVehicleModel(vehicle, polmav)
	end

	if polmav2 then
		return IsVehicleModel(vehicle, polmav2)
	end	

	if polmav3 then
		return IsVehicleModel(vehicle, polmav3)
	end	
end
