...
AddEventHandler("RefuelCar",function()
    ...
    if curw == w then
        ...
        if DoesEntityExist(targetVehicle) then
            ...
            exports['mythic_notify']:SendAlert('info', 'rempli')
        else
            exports['mythic_notify']:SendAlert('erreur', 'aucun reservoir')
        end
    else
        exports['mythic_notify']:SendAlert('erreur', 'Vous avez besoin d’un gallon!')
    end
end)
...
AddEventHandler("RefuelCarServerReturn",function()
    ...
    refillVehicle()
    local finished = exports["taskbar"]:taskBar(timer,"Remplissage")
    ...
    endanimation()
    exports['mythic_notify']:SendAlert('succes', 'Vous avez payé $' .. round(costs) .. ' pour acheter de l’essence.')
end)
...
AddEventHandler("CarFuelAlarm",function()
    if not alarmset then
        alarmset = true
        local i = 5
        exports['mythic_notify']:SendAlert('erreur', 'Nous sommes à court d’essence!')
        while i > 0 do
    ...
    if car ~= 0 and (wasInCar or IsCar(car)) then
     ...
     if IsControlJustReleased(0, 29) and IsPedInAnyVehicle(PlayerPedId()) then
        ...
        not IsThisModelAPlane(GetEntityModel(GetVehiclePedIsIn(GetPlayerPed(-1),false))) then
            if seatbelt == false then
                seatbelt = true
                exports['mythic_notify']:SendAlert('inform', 'Ceinture attacher!')
                TriggerEvent("InteractSound_CL:PlayOnOne","seatbelt",0.1)
            else
                TriggerEvent("InteractSound_CL:PlayOnOne","seatbeltoff",0.7)
                exports['mythic_notify']:SendAlert('info', 'La ceinture a été enlevée!')
                seatbelt = false
            end
        else
            exports['mythic_notify']:SendAlert('erreur', 'Vous ne pouvez pas porter une ceinture dans cette voiture!')
        end
    end
     if (GetPedInVehicleSeat(car, -1) == ped) then
         ...
         if IsControlJustReleased(0, Keys['M']) then
             if cruiseIsOn then
                exports['mythic_notify']:SendAlert('info', 'Stabilisation de la vitesse desactiver')
             else
                exports['mythic_notify']:SendAlert('info', 'Stabilisation de la vitesse Active')
             end
    ...
    while true do
        ...
        if bool == true then
            ...
            if DoesEntityExist(veh) and IsEntityAVehicle(veh) and #(GetEntityCoords(veh) - GetEntityCoords(PlayerPedId())) < 5.0 then
                ...
                info = string.format("Bas ~g~"..Controlkey["generalUse"][2].."~s~ Pour faire le plein d’essence | ~g~$%s + Taxe", costs)
                ...
                if IsControlJustPressed(1, Controlkey["generalUse"][1]) then
                    if curFuel >= 100 then
                        ...
                        exports['mythic_notify']:SendAlert('erreur', 'Votre réservoir d’essence est plein')
                    else
