...
function StopNPCJob(cancel)
  ...
  if cancel then
    ...
    TriggerServerEvent('esx_bennysjob:onNPCJobCompleted')
  end
...
function OpenbennysHarvestMenu()
  ...
  if Config.EnablePlayerManagement and PlayerData.job ~= nil and PlayerData.job.grade_name ~= 'recrue' then
    ...
    ESX.UI.Menu.Open(
      'default', GetCurrentResourceName(), 'bennys_harvest',
      {
        title    = _U('harvest'),
		align    = 'bottom-right',
        elements = elements
      ...
      function(data, menu)
        if data.current.value == 'gaz_bottle' then
          menu.close()
          TriggerServerEvent('esx_bennysjob:startHarvest')
        end

        if data.current.value == 'fix_tool' then
          menu.close()
          TriggerServerEvent('esx_bennysjob:startHarvest2')
        end

        if data.current.value == 'caro_tool' then
          menu.close()
          TriggerServerEvent('esx_bennysjob:startHarvest3')
        end
...
function OpenbennysCraftMenu()
  if Config.EnablePlayerManagement and PlayerData.job ~= nil and PlayerData.job.grade_name ~= 'recrue' then
    ...
    ESX.UI.Menu.Open(
      'default', GetCurrentResourceName(), 'bennys_craft',
      {
        title    = _U('craft'),
		align    = 'bottom-right',
        elements = elements
      ...
      function(data, menu)
        if data.current.value == 'blow_pipe' then
          menu.close()
          TriggerServerEvent('esx_bennysjob:startCraft')
        end

        if data.current.value == 'fix_kit' then
          menu.close()
          TriggerServerEvent('esx_bennysjob:startCraft2')
        end

        if data.current.value == 'caro_kit' then
          menu.close()
          TriggerServerEvent('esx_bennysjob:startCraft3')
        end
          ...
          function(data, menu)
            ...
            if amount == nil or amount < 0 then
              ...
              if closestPlayer == -1 or closestDistance > 3.0 then
                ...
                TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_bennys', _U('bennys'), amount)
              end
            end
          end,
        function(data, menu)
...
RegisterNetEvent('esx_bennysjob:onHijack')
AddEventHandler('esx_bennysjob:onHijack', function()
  local playerPed = GetPlayerPed(-1)
  local coords    = GetEntityCoords(playerPed)
  ...
  while true do
    ...
    if distance ~= -1 and distance <= 3.0 then
      ...
      if LastEntity ~= entity then
        TriggerEvent('esx_bennysjob:hasEnteredEntityZone', entity)
        LastEntity = entity
      end
      ...
      if LastEntity ~= nil then
        TriggerEvent('esx_bennysjob:hasExitedEntityZone', LastEntity)
        LastEntity = nil
      end
...
Citizen.CreateThread(function()
    while true do
        ...
        if IsControlJustReleased(0, Keys['F6']) and GetLastInputMethod(2) and PlayerData.job ~= nil and PlayerData.job.name == 'bennys' then
            OpenMobilebennysActionsMenu()
        end

        if IsControlJustReleased(0, Keys['DELETE']) and GetLastInputMethod(2) and PlayerData.job ~= nil and PlayerData.job.name == 'bennys' then
