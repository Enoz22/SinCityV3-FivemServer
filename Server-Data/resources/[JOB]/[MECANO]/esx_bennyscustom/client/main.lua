...
AddEventHandler('esx:playerLoaded', function(xPlayer)
	...
	ESX.TriggerServerCallback('esx_bennyscustom:getVehiclesPrices', function(vehicles)
		Vehicles = vehicles
	end)
end)
...
RegisterNetEvent('esx_bennyscustom:installMod')
AddEventHandler('esx_bennyscustom:installMod', function()
	local vehicle = GetVehiclePedIsIn(PlayerPedId(), false)
	myCar = ESX.Game.GetVehicleProperties(vehicle)
	TriggerServerEvent('esx_bennyscustom:refreshOwnedVehicle', myCar)
end)

RegisterNetEvent('esx_bennyscustom:cancelInstallMod')
AddEventHandler('esx_bennyscustom:cancelInstallMod', function()
	local vehicle = GetVehiclePedIsIn(PlayerPedId(), false)
...
function OpenLSMenu(elems, menuName, menuTitle, parent)
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), menuName,
	{
		...
		for k,v in pairs(Config.Menus) do
			...
			if k == data.current.modType or isRimMod then
				...
				if data.current.label == _U('by_default') or string.match(data.current.label, _U('installed')) then
					...
					TriggerEvent('esx_bennyscustom:installMod')
				else
					...
					if isRimMod then
						...
						TriggerServerEvent('esx_bennyscustom:buyMod', price)
		...
		menu.close()
		TriggerEvent('esx_bennyscustom:cancelInstallMod')
...
Citizen.CreateThread(function()
	while true do
		...
		if IsPedInAnyVehicle(playerPed, false) then
			...
			if (PlayerData.job and PlayerData.job.name == 'bennys') or not Config.IsBennysJobOnly then
				for k,v in pairs(Config.Zones) do
