Locales['fr'] = {
    -- Cloakroom
    ['cloakroom']                = 'Vestiaire',
    ['citizen_wear']             = 'Tenue civile',
    ['pizza_outfit']             = 'Tenue pizzeria',
    ['no_outfit']                = 'Il n\'y a pas d\'uniforme à votre taille...',
    ['open_cloackroom']          = '~INPUT_CONTEXT~ pour vous changer',

    -- Vault  
    ['get_weapon']               = 'Prendre Arme',
    ['put_weapon']               = 'Déposer Arme',
    ['get_weapon_menu']          = 'Coffre - Prendre Arme',
    ['put_weapon_menu']          = 'Coffre - Déposer Arme',
    ['get_object']               = 'Prendre Objet',
    ['put_object']               = 'Déposer Objet',
    ['vault']                    = 'Stock / Frigo',
    ['open_vault']               = '~INPUT_CONTEXT~ pour accéder au coffre',

    -- Fridge  
    ['get_object']               = 'Prendre Objet',
    ['put_object']               = 'Déposer Objet',
    ['fridge']                   = 'Frigo',
    ['open_fridge']              = '~INPUT_CONTEXT~ pour accéder au frigo',
    ['pizza_fridge_stock']       = 'Frigo du pizzeria',
    ['fridge_inventory']         = 'Contenu du frigo',

    -- Shops  
    ['shop']                     = 'Boutique de la pizzeria',
    ['shop_menu']                = '~INPUT_CONTEXT~ pour accéder à la boutique.',
    ['bought']                   = 'Vous avez acheté ~b~1x ',
    ['not_enough_money']         = 'Vous n\'avez ~r~pas assez~s~ d\'argent.',
    ['max_item']                 = 'Vous en portez déjà assez sur vous.',

    -- Vehicles  
    ['vehicle_menu']             = 'Véhicule',
    ['vehicle_out']              = 'Il y a déja un véhicule dehors',
    ['vehicle_spawner']          = '~INPUT_CONTEXT~ pour sortir un véhicule',
    ['store_vehicle']            = '~INPUT_CONTEXT~ pour ranger le véhicule',
    ['service_max']              = 'Service complet : ',
    ['spawn_point_busy']         = 'Un véhicule occupe le point de sortie',

    -- Boss Menu  
    ['take_company_money']       = 'Retirer argent société',
    ['deposit_money']            = 'Déposer argent',
    ['amount_of_withdrawal']     = 'Montant du retrait',
    ['invalid_amount']           = 'Montant invalide',
    ['amount_of_deposit']        = 'Montant du dépôt',
    ['open_bossmenu']            = '~INPUT_CONTEXT~ pour ouvrir le menu',
    ['invalid_quantity']         = 'Quantité invalide',
    ['you_removed']              = 'Vous avez retiré x',
    ['you_added']                = 'Vous avez ajouté x',
    ['quantity']                 = 'Quantité',
    ['inventory']                = 'Inventaire',
    ['pizza_stock']              = 'Stock pizzeria',

    -- Billing Menu  
    ['billing']                  = 'Facture',
    ['no_players_nearby']        = 'Aucun joueur à proximité',
    ['billing_amount']           = 'Montant de la facture',
    ['amount_invalid']           = 'Montant invalide',

    -- Crafting Menu  
    ['crafting']                = 'Fabrication pizzeria',
    ['ing']                     = 'Ingredient pour pizza',
    ['pate']                    = 'Pate a pizza',
    ['pizza1']                  = 'Pizza reine',
    ['pizza2']                  = 'Pizza au 5 fromages',
    ['pizza3']                  = 'Pizza au barbecue',
    ['pizza4']                  = 'Pizza au saumon',
    ['pizza5']                  = 'Pizza orientale',
    ['pizza6']                  = 'Pizza calzone',
    ['assembling_cocktail']     = 'Mélange des différents ingrédients en cours !',
    ['craft_miss']              = 'Malheureux échec du mélange...',
    ['not_enough']              = 'Pas assez d\'~r~ ingredient ou argent',
    ['craft']                   = '~g~ Vous avez terminé de faire la pizza ',
    ['barbecue']                = 'barbecue',
    ['orientale']               = 'orientale',
    ['5fromages']               = '5 fromages',
    ['saumons']                 = 'saumons',
    ['reine']                   = 'reine',
    ['calzone']                 = 'calzone',          
    -- Misc  
    ['map_blip']                = 'Crazy Pizza',
    ['pizza']                   = 'Crazy Pizza',
    -- Phone  
    ['pizza_phone']             = 'Crazy Pizza',
    ['pizza_customer']          = 'Citoyen', 


    ['Acheter_ingredient']      = 'Acheter Ingredients',
    ['Acheter_pâtes']           = 'Acheter Pâtes',

}
    

