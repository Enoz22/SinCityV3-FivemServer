...
function OpenGetStocksMenu()
	ESX.TriggerServerCallback('esx_yellowjack:getStockItems', function(items)
		local elements = {}
		for i=1, #items, 1 do
				...
				ESX.UI.Menu.Open(
					...
					function(data2, menu2)
						...
						if count == nil or count <= 0 then
							...
							TriggerServerEvent('esx_yellowjack:getStockItem', itemName, count)
						end
...
function OpenPutStocksMenu()
	...
	ESX.TriggerServerCallback('esx_yellowjack:getPlayerInventory', function(inventory)
				...
				ESX.UI.Menu.Open(
					...
					function(data2, menu2)
						...
						if count == nil or count <= 0 then
							...
							TriggerServerEvent('esx_yellowjack:putStockItems', itemName, count)
						end
...
AddEventHandler('esx_yellowjack:hasEnteredMarker', function(zone)
...
AddEventHandler('esx_yellowjack:hasExitedMarker', function(zone)
	ESX.UI.Menu.CloseAll()
	if (zone == 'Recolte') and PlayerData.job ~= nil and PlayerData.job.name == Config.NameJob then
		TriggerServerEvent('esx_yellowjack:stopHarvest')
	end  
	if (zone == 'Traitement') and PlayerData.job ~= nil and PlayerData.job.name == Config.NameJob then
		TriggerServerEvent('esx_yellowjack:stopTransform')
	end
	if (zone == 'Vente') and PlayerData.job ~= nil and PlayerData.job.name == Config.NameJob then
		TriggerServerEvent('esx_yellowjack:stopSell')
	end
	CurrentAction = nil
end)
	...
	while true do
		...
		if PlayerData.job ~= nil and PlayerData.job.name == Config.NameJob then
			...
			if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
				...
				TriggerEvent('esx_yellowjack:hasEnteredMarker', currentZone)
			end

			if not isInMarker and HasAlreadyEnteredMarker then
				HasAlreadyEnteredMarker = false
				TriggerEvent('esx_yellowjack:hasExitedMarker', LastZone)
			end
		end
	end
end)
		...
		if CurrentAction ~= nil then
			...
			if IsControlPressed(0,  Keys['E']) and PlayerData.job ~= nil and PlayerData.job.name == Config.NameJob and (GetGameTimer() - GUI.Time) > 300 then
				if CurrentAction == 'harvest' then
					TriggerServerEvent('esx_yellowjack:startHarvest', CurrentActionData.zone)
				end
				if CurrentAction == 'traitement' then
					TriggerServerEvent('esx_yellowjack:startTransform', CurrentActionData.zone)
				end
				...
				if CurrentAction == 'sell' then
					TriggerServerEvent('esx_yellowjack:startSell', CurrentActionData.zone)
				end
