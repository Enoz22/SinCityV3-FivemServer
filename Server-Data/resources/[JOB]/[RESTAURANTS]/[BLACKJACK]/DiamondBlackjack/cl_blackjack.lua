...
cfg.blackjackTables = {
    --[id] = {x,y,z,heading}
    [0] = {
        dealerPos = vector3(455.5,-255.99,71.24),
        dealerHeading = 119.80,
        -- tablePos = vector3(455.2,-256.17,70.26),
        tablePos = vector3(455.03,-256.25,70.26),
        tableHeading = 285.0,
        distance = 1000.0,
    ...
    [1] = {
        ...
        dealerPos = vector3(454.91,-252.63,71.24),
        dealerHeading = 92.09,
        tablePos = vector3(454.12,-252.62,70.26),
        tableHeading = 272.0,
        distance = 1000.0,
        prop = "vw_prop_casino_blckjack_01"
    },
            ...
            if closestChair ~= nil and closestChairDist < 2 then
                if not timeoutHowToBlackjack then
                    if blackjackTableData[closestChair] == false then 
                        drawNativeNotification("Appuyez sur ~INPUT_PICKUP~ pour jouer au blackjack")
                    else 
                        drawNativeNotification("Ce siège est pris.")
                    end
...
Citizen.CreateThread(function()
    while true do 
        ...
        if drawCurrentHand then 
            ...
            AddTextComponentString("Votre main ~b~["..tostring(currentHand).."]~w~ | Main du croupier ~r~["..tostring(dealersHand).."]")
    ...
    while true do 
        if waitingForBetState then
            if IsDisabledControlJustPressed(0, 22) then --Custom Bet [space]
                local tmpInput = getGenericTextInput("Montant du pari 'Maximum : 15.000 Jetons'")
    ...
    while true do
        if blackjackInstructional ~= nil then 
            ...
            if waitingForBetState then 
                ...
                DrawAdvancedNativeText(0.991, 0.935, 0.005, 0.0028, 0.29, "PARI:", 255, 255, 255, 255, 0, 0)
        ...
        if drawTimerBar then
            ...
            DrawAdvancedNativeText(1.013, 0.892, 0.005, 0.0028, 0.29, "TEMPS:", 255, 255, 255, 255, 0, 0)
            if timeLeft < 10 then
...
AddEventHandler("Blackjack:beginBetsBlackjack",function(gameID,tableId)
    ...
    ClearHelp(true)
    drawNativeNotification("Faites vos jeux")
    ...
    Citizen.CreateThread(function()
        ...
        if not bettedThisRound then
            ...
            drawNativeNotification("Pas de pari placé, round sauté")
        end
    end)
end)
...
function goToBlackjackSeat(blackjackSeatID)
    ...
    drawNativeNotification("En attendant le début du prochain jeu...")
    blackjackAnimsToLoad = {
      "anim_casino_b@amb@casino@games@blackjack@dealer",
      "anim_casino_b@amb@casino@games@shared@dealer@",
      "anim_casino_b@amb@casino@games@blackjack@player",  
...
AddEventHandler("Blackjack:blackjackLose",function(tableId)
    if closeToCasino then
        ...
        angryILost()
        canExitBlackjack = true
        drawNativeNotification("~r~Vous avez perdu !")
        drawCurrentHand = false
        currentHand = 0
        dealersHand = 0
    end
...
AddEventHandler("Blackjack:blackjackPush",function(tableId)
    if closeToCasino then
        ...
        annoyedIPushed()
        canExitBlackjack = true
        drawNativeNotification("~o~Vous avez fait égalité !")
        drawCurrentHand = false
        currentHand = 0
        dealersHand = 0
    end
...
AddEventHandler("Blackjack:blackjackWin",function(tableId)
    if closeToCasino then
        ...
        happyIWon()
        canExitBlackjack = true
        drawNativeNotification("~g~Vous avez gagnez !")
        drawCurrentHand = false
        currentHand = 0
        dealersHand = 0
    end
...
AddEventHandler("Blackjack:chipsCleanup",function(chairId,tableId)
    if closeToCasino then
        if string.sub(chairId, -5) ~= "jetons" then
            dealerPed = getDealerFromTableId(tableId)
            local gender = getDealerGenderFromPed(dealerPed)
            if gender == "male" then 
                genderAnimString = "" 
...
function betChipsForNextHand(chipsAmount,chipsProp,something,chairID,someBool,zOffset)
    ...
    if cardObjects[tostring(chairID) .. "jetons"] ~= nil then
        table.insert(cardObjects[tostring(chairID) .. "jetons"],chipsObj)
    else 
        cardObjects[tostring(chairID) .. "jetons"] = {}
        table.insert(cardObjects[tostring(chairID) .. "jetons"],chipsObj)
    end
...
function setupBlackjackMidBetScaleform(scaleform)
    ...
    PushScaleformMovieFunctionParameterInt(1)
    Button(GetControlInstructionalButton(2, 194, true)) -- The button to display
    ButtonMessage("~g~Rester") --BACKSPACE
    PopScaleformMovieFunctionVoid()

    PushScaleformMovieFunction(scaleform, "SET_DATA_SLOT")
    PushScaleformMovieFunctionParameterInt(4)
    Button(GetControlInstructionalButton(2, 22, true))
    ButtonMessage("Demander une carte") --Space
    PopScaleformMovieFunctionVoid()  
