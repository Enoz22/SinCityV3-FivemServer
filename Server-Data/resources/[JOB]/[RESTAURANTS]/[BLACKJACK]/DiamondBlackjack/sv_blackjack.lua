...
function tryTakeChips(source,amount)
	...
	local item = xPlayer.getInventoryItem('jeton').count
		if item >= amount then
            xPlayer.removeInventoryItem('jeton', amount)
            TriggerEvent('esx_addonaccount:getSharedAccount', 'society_poolbar', function (account)
                account.addMoney(amount)
        end)
			return true
		else 
			TriggerClientEvent('esx:showNotification', source, 'Vous n\'avez pas assez de jetons')
...
AddEventHandler("Blackjack:requestSitAtBlackjackTable", function(chairId)
    ...
    if source ~= nil then
        ...
        TriggerClientEvent("blackjack:notify",source,"~r~Erreur, vous ne pouvez pas vous asseoir.")
    ...
    if gameId ~= nil and betAmount ~= nil and chairId ~= nil then 
        ...
        if not blackjackGameInProgress[gameId] then
            if tonumber(betAmount) then
                ...
                if betAmount > 500 then
                    TriggerClientEvent("blackjack:notify",source,"~r~Les paris au dessus de 500 ne sont pas accepté")
                else 
                    if tryTakeChips(source,betAmount) then
                        ...
                        TriggerClientEvent("blackjack:notify",source,"~g~Pari placé: " .. tostring(betAmount) .. " jetons.")
                    end
                end
            end
        end
    else
        TriggerClientEvent("blackjack:notify",source,"~r~Erreur sur le pari")
    end
end)
                ...
                if blackjackGameData[gameId] ~= nil then
                    ...
                    if not isTableEmpty(blackjackGameData[gameId]) then
                                        ...
                                        while nextCardCount >= 1 do
                                            ...
                                            if blackjackGameData[gameId][source][2][nextCardCount] == true then --if hit 
                                                ...
                                                if currentHand > 21 then
                                                    ...
                                                    TriggerClientEvent("blackjack:notify",source,"~r~-"..tostring(lostAmount).." jetons")
                                                    if lostAmount > 10000000 then
                                                        TriggerClientEvent('chatMessage', -1, "Diamond Casino | " .. GetPlayerName(source) .. " a perdu " .. tostring(getMoneyStringFormatted(lostAmount)) .. " jetons !")
                                                    end
                                                ...
                                                if dealerHand > 21 then
                                                    ...
                                                    if playerPing ~= nil then
                                                        if playerPing > 0 then
                                                            ...
                                                            TriggerClientEvent("blackjack:notify",source,"~g~+"..tostring(potentialWinAmount).." jetons")
                                                            if potentialPushAmount > 10000000 then
                                                                TriggerClientEvent('chatMessage', -1, "Diamond Casino | " .. GetPlayerName(source) .. " a GAGNÉ " .. tostring(getMoneyStringFormatted(potentialPushAmount)) .. " jetons !")
                                                            end
                                                        end
                                                    end
                                                ...
                                                elseif currentHand == dealerHand then
                                                    ...
                                                    if playerPing ~= nil then
                                                        if playerPing > 0 then
                                                            ...
                                                            TriggerClientEvent("blackjack:notify",source,"~b~+0 jetons")
                                                        end
                                                    end
                                                else
                                                    ...
                                                    if playerPing ~= nil then
                                                        if playerPing > 0 then
                                                            ...
                                                            TriggerClientEvent("blackjack:notify",source,"~r~-"..tostring(potentialPushAmount).." jetons")
                                                            if potentialPushAmount > 10000000 then
                                                                TriggerClientEvent('chatMessage', -1, "Diamond Casino | " .. GetPlayerName(source) .. " ha PERDU " .. tostring(getMoneyStringFormatted(potentialPushAmount)) .. " jetons !")
                                                            end
                                                        end
                                                    end
                                                end
                        ...
                        for chairID=chairIdInitial,chairIdFinal do
                            if blackjackTables[chairID] ~= false then
                                ...
                                if blackjackGameData[gameId][source] ~= nil then 
                                    ...
                                    TriggerClientEvent("Blackjack:chipsCleanup",-1,tostring(chairID).."jetons",tableId)
                        ...
                        for chairID=chairIdInitial,chairIdFinal do
                            ...
                            TriggerClientEvent("Blackjack:chipsCleanupNoAnim",-1,tostring(chairID).."jetons",tableId)
                        end
                        blackjackGameInProgress[gameId] = false
                    else 
