...
local function interact()
    ...
    if (Animal) then
        local message = Language['depessing']
        ...
        if (Config.useProgresseBar) then exports['progressBars']:startUI(Animal.butcheringTime * 1000, Language['progressBarMessage']) end
...
AddEventHandler('dsp_hunting:placingBait', function()
    ...
    baitPosition = {x = pos.x, y=pos.y}
        ...
        if not IsAnimated then
            IsAnimated = true
            if (Config.useProgresseBar) then exports['progressBars']:startUI(7000, 'Pose de l\'âppat en cours...') end
            TaskPlayAnim(PlayerPedId(), "anim@gangops@facility@servers@bodysearch@" ,"player_search" ,8.0, -8.0, -1, 48, 2, false, false, false)
            TaskPlayAnim(PlayerPedId(), "amb@medic@standing@kneel@base" ,"base" ,8.0, -8.0, -1, 1, 2, false, false, false)
        end
        ...
        if targetAnimal ~= 0 then
            if IsPedDeadOrDying(targetAnimal)then
                ...
                if targetAnimal ~= oldAnimal and not depessing and (pedType == 28) then
                    ...
                    DrawText3Ds(pos.x, pos.y, pos.z, Language["actionMessage"])

                    if IsControlJustPressed(1, Config.keys['interact']) then
                        if (Config.useWeapon) then
                            if (hasValidWeapon(GetSelectedPedWeapon(PlayerPedId()))) then
                                interact()
                            else
                                showNotification(Language["notKnife"])
                            end
                        else
                            interact()
                        end
