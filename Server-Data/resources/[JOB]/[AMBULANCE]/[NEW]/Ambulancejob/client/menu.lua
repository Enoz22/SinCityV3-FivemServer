...
AddEventHandler("AppelemsTropBien", function(coords, id)
	...
	AppelCoords = coords
	AppelID = id
	ESX.ShowAdvancedNotification("EMS", "~b~Demande d'EMS", "Quelqu'un à besoin d'un ems ! Ouvrez votre menu ~b~[F6] ~s~pour intervenir", "CHAR_CALL911", 8)
end)
            ...
            CreateThread(function()
				...
				RageUI.IsVisible(RMenu:Get('ambulance', 'main'), true, true, true, function()
					RageUI.Checkbox("Prendre/Quitter son service", nil, ambuservice, { Style = RageUI.CheckboxStyle.Tick }, function(Hovered, Selected, Active, Checked)
						...
						if ambuservice then
							...
							RageUI.ButtonWithStyle("[~y~ANNONCE~s~] :~o~ EMS EN SURCHARGE", nil, {RightLabel = "→→"}, true, function(Hovered, Active, Selected)
								if (Selected) then
									TriggerServerEvent('EMS:Surcharge')
								end
							end)
				...
				RageUI.IsVisible(RMenu:Get('ambulance', 'lister'), true, true, true, function()
					...
					for numreport = 1, #reportlistesql, 1 do
					  RageUI.ButtonWithStyle("[~y~Appel en attente ~s~] - ~b~Numéro ~s~: "..reportlistesql[numreport].id, nil, { RightLabel = "~r~En ATTENTE"}, true, function(Hovered, Active, Selected)
						  if (Selected) then
...
Citizen.CreateThread(function()
    while true do
       ...
       RageUI.IsVisible(RMenu:Get('ambulance', 'gestr'), true, true, true, function()
        ...
        RageUI.Separator("~s~↓~b~ Gestion~s~ ↓")
								...
								RageUI.ButtonWithStyle(infos.label,nil, {RightBadge = RageUI.BadgeStyle.Car}, ESX.PlayerData.job.grade >= infos.minimum_grade, function(_,_,s)
									if s then
										Citizen.CreateThread(function()
											...
											local vehicle = CreateVehicle(model, 334.07, -572.57, 28.19, 339.23 ,true, false)
								...
								RageUI.ButtonWithStyle(infos.label,nil, {RightBadge = RageUI.BadgeStyle.Car}, ESX.PlayerData.job.grade >= infos.minimum_grade, function(_,_,s)
									if s then
										Citizen.CreateThread(function()
											...
											RequestModel(model)
											while not HasModelLoaded(model) do Citizen.Wait(1) end
											local vehicle = CreateVehicle(model, 351.27, -588.93, 74.55, 337.57,true, false)
											SetModelAsNoLongerNeeded(model)
											SetVehicleEngineOn(vehicle, true, true, false)
											lastVehicle = vehicle
											SetVehicleLivery(lastVehicle, 1)
										end)
									end
								end)
							end
...
RMenu:Get('pharma', 'main'):SetSubtitle("~b~Récupérer votre équipement")
			...
			Citizen.Wait(1)
				...
				RageUI.IsVisible(RMenu:Get('pharma', 'main'), true, true, true, function()
						for index,infos in pairs(Config.Hospitals.CentralLosSantos.Pharmacies) do
							RageUI.ButtonWithStyle("Medikit ~b~(x5)", nil, {RightBadge = RageUI.BadgeStyle.Heart}, true, function(_,_,s)
								if s then
						...
						for index,infos in pairs(Config.Hospitals.CentralLosSantos.Pharmacies) do
							RageUI.ButtonWithStyle("Bandages ~b~(x5)", nil, {RightBadge = RageUI.BadgeStyle.Heart}, true, function(_,_,s)
								if s then
