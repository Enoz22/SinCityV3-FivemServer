...
----------------- COMMANDE POUR REVIVE EN TANT QU'ADMIN ----------------- 
...
AddEventHandler('Pharmacy:giveItemm', function(itemName, itemLabel)
	...
	local qtty = xPlayer.getInventoryItem(itemName).count
		...
		if qtty < 15 then
			xPlayer.addInventoryItem(itemName, 5)
			TriggerClientEvent('acore:notif', _source, '~g~Ambulance\n~w~Tu as recu des Medikit ~o~(15 Maximum)')
		else
			TriggerClientEvent('acore:notif', _source, '~g~Ambulance\n~r~Maximum de bandages Atteints')
		end
	end)
...
RegisterServerEvent('EMS:Surcharge')
AddEventHandler('EMS:Surcharge', function()
    local xPlayer = ESX.GetPlayerFromId(_source)
    local xPlayers    = ESX.GetPlayers()
    for i=1, #xPlayers, 1 do
        local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
        TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], 'EMS', '~b~Annonce EMS', 'Les EMS sont actuellement ~o~surchargés d\'appels !', 'CHAR_CALL911', 7)
    end
end)
