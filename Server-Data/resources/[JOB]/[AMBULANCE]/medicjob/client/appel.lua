...
local AppelPris = true
local AppelDejaPris = true
...
AddEventHandler("AppelemsTropBien", function(coords, id)
	...
	AppelID = id
	print(AppelID)
	exports["core_dispach"]:addCall2("Quelqu'un à besoin d'un ems",coords,"ambulance",nil,nil,nil,"number") -- oki mes vide
	afficherTextVolant(coords, id)
     ...
     while rea do
          if GetDistanceBetweenCoords(XAcoords, GetEntityCoords(GetPlayerPed(-1))) < 10.0 then
			...
			--    Draw3DText(XAcoords.x, XAcoords.y, XAcoords.z, "~g~Une personne à besoin d'une réanimation ici ...\n~b~[E]~w~Pour la réanimer.", 4, 0.1, 0.1)
               Draw3DText(XAcoords.x, XAcoords.y, XAcoords.z, "~b~[E]~w~Pour la réanimer.", 4, 0.1, 0.1)
