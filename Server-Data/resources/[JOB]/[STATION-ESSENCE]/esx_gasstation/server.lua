...
AddEventHandler(
    ...
    function(r, p)
        ...
        if b[source] == nil then
            ...
            local t = s.identifier
            if t then
                local z = math.floor(tonumber(p.amount))
                if z and z > 0 then
                    if Config.ESX.account_stores == "money" or Config.ESX.account_stores == "cash" then
                        money = s.getMoney()
                    else
                    ...
                    if money >= z then
                        if Config.ESX.account_stores == "money" or Config.ESX.account_stores == "cash" then
                            s.removeMoney(Config.ESX.account_stores, z)
                        else
                            s.removeAccountMoney(Config.ESX.account_stores, z)
                        end
                        giveMarketMoney(r, z)
                        insertBalanceHistory(r, 0, Lang[Config.lang]["money_deposited"], z)
                        TriggerClientEvent(
                            "gas_station:Notify",
                            source,
                            "sucesso",
                            Lang[Config.lang]["money_deposited"]
                        )
                        openUI(source, r, true)
                    else
                        TriggerClientEvent(
...
function giveMarketMoney(G, z)
    local o = "UPDATE `gas_station_business` SET money = money + @amount WHERE gas_station_id = @gas_station_id"
    MySQL.Sync.execute(o, {["@amount"] = z, ["@gas_station_id"] = G})
end
function tryGetMarketMoney(G, z)
    local o = "SELECT money FROM `gas_station_business` WHERE gas_station_id = @gas_station_id"
    local query = MySQL.Sync.fetchAll(o, {["@gas_station_id"] = G})[1]
    if tonumber(query.money) >= z then
        local o =
        ...
        MySQL.Sync.execute(o, {["@money"] = tonumber(query.money) - z, ["@amount"] = z, ["@gas_station_id"] = G})
        return true
    else
        return false
    end
