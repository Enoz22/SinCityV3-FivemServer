		...
		if galao or output.stock > amount then
			...
			if money >= price then
				...
				if galao then
					...
					TriggerClientEvent("gas_station:Notify",source,"sucesso","Paiement de <b>$"..price.." </b> pour "..amount.." litres.",8000)
				end
			else
				TriggerClientEvent('vrp_legacyfuel2:insuficiente',source,vehicle,fuel2)
				TriggerClientEvent("gas_station:Notify",source,"negado","Argent insuffisant.",8000)
			end
		else
			TriggerClientEvent('vrp_legacyfuel2:insuficiente',source,vehicle,fuel2)
			TriggerClientEvent("gas_station:Notify",source,"negado","Il n'y'a pas suffisement de stock (Max: "..output.stock.." litres).",8000)
		end
	end
end)
