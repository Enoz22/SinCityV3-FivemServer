...
function OpenCloakroomMenu()
  ...
  if PlayerData.job.grade_name == 'recruit' then
    table.insert(elements, {label = _U('fbi_wear'), value = 'cadet_wear'})
  end
  ...
  if PlayerData.job.grade_name == 'sergeant' then
    table.insert(elements, {label = _U('fbi_wear'), value = 'sergeant_wear'})
  end

  if PlayerData.job.grade_name == 'lieutenant' then
    table.insert(elements, {label = _U('fbi_wear'), value = 'lieutenant_wear'})
  end

  if PlayerData.job.grade_name == 'boss' then
    table.insert(elements, {label = _U('fbi_wear'), value = 'commandant_wear'})
  end
  ...
  ESX.UI.Menu.Open(
    ...
    function(data, menu)
      ...
      if
        ...
        data.current.value == 'fbi_wear' or
  ...
  if Config.EnableArmoryManagement then
    ...
    ESX.UI.Menu.Open(
      ...
      function(data, menu)
        ...
        TriggerServerEvent('esx_fbijob:giveWeapon', weapon,  1000)
      end,
      function(data, menu)
    ...
    ESX.TriggerServerCallback('esx_society:getVehiclesInGarage', function(garageVehicles)
      ...
      ESX.UI.Menu.Open(
        ...
        function(data, menu)
          ...
          TriggerServerEvent('esx_society:removeVehicleFromGarage', 'fbi', vehicleProps)
...
function OpenFbiActionsMenu()
	...
	ESX.UI.Menu.Open(
	'default', GetCurrentResourceName(), 'fbi_actions',
	{
		title    = 'Fbi',
		align    = 'top-left',
		elements = {
			...
			'default', GetCurrentResourceName(), 'citizen_interaction',
			{
				...
				if closestPlayer ~= -1 and closestDistance <= 3.0 then
					...
					elseif action == 'handcuff' then
						TriggerServerEvent('esx_fbijob:handcuff', GetPlayerServerId(closestPlayer))
...
function OpenIdentityCardMenu(player)
  ...
  if Config.EnableESXIdentity then
    ...
    ESX.TriggerServerCallback('esx_fbijob:getOtherPlayerData', function(data)
    ...
    ESX.UI.Menu.Open(
      ...
      function(data, menu)
        ...
        if data.current.value ~= nil then
          ...
          TriggerServerEvent('esx_fbijob:confiscatePlayerItem', GetPlayerServerId(player), itemType, itemName, amount)
...
function OpenFineCategoryMenu(player, category)
  ...
  ESX.TriggerServerCallback('esx_fbijob:getFineList', function(fines)
    ...
    ESX.UI.Menu.Open(
      ...
      function(data, menu)
        ...
        if Config.EnablePlayerManagement then
          TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(player), 'society_fbi', _U('fine_total', label), amount)
        else
...
function ShowPlayerLicense(player)
	...
	ESX.TriggerServerCallback('esx_fbijob:getOtherPlayerData', function(data)
		...
		function(data, menu)
			...
			TriggerServerEvent('esx_fbijob:message', GetPlayerServerId(player), _U('license_revoked', data.current.label))
...
function OpenVehicleInfosMenu(vehicleData)
  ...
  ESX.TriggerServerCallback('esx_fbijob:getVehicleInfos', function(infos)
...
function OpenGetStocksMenu()
  ...
  ESX.TriggerServerCallback('esx_fbijob:getStockItems', function(items)
...
AddEventHandler('esx:setJob', function(job)
	...
	Citizen.Wait(5000)
	TriggerServerEvent('esx_fbijob:forceBlip')
end)
...
AddEventHandler('esx_fbijob:hasEnteredEntityZone', function(entity)
...
AddEventHandler('esx_fbijob:hasExitedEntityZone', function(entity)
...
RegisterNetEvent('esx_fbijob:handcuff')
AddEventHandler('esx_fbijob:handcuff', function()
...
AddEventHandler('esx_fbijob:drag', function(cop)
...
AddEventHandler('esx_fbijob:OutVehicle', function(t)
  ...
  while true do
    ...
    if closestDistance ~= -1 and closestDistance <= 3.0 then
      ...
      if LastEntity ~= closestEntity then
        TriggerEvent('esx_fbijob:hasEnteredEntityZone', closestEntity)
      ...
      if LastEntity ~= nil then
        TriggerEvent('esx_fbijob:hasExitedEntityZone', LastEntity)
        LastEntity = nil
      end
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction ~= nil then
			...
			if IsControlJustReleased(0, Keys['E']) and PlayerData.job ~= nil and PlayerData.job.name == 'fbi' then
				...
				elseif CurrentAction == 'delete_vehicle' then
					if Config.EnableSocietyOwnedVehicles then
						...
						TriggerServerEvent('esx_society:putVehicleInGarage', 'fbi', vehicleProps)
					else
						if GetEntityModel(vehicle) == GetHashKey('fbi') or
							GetEntityModel(vehicle) == GetHashKey('fbi2') or
							GetEntityModel(vehicle) == GetHashKey('bcsoT') or
							GetEntityModel(vehicle) == GetHashKey('riot') or
							GetEntityModel(vehicle) == GetHashKey('bcsob') or
							GetEntityModel(vehicle) == GetHashKey('bcsot')
						then
...
RegisterNetEvent('esx_fbijob:updateBlip')
AddEventHandler('esx_fbijob:updateBlip', function()
...
AddEventHandler('playerSpawned', function(spawn)
	...
	if not hasAlreadyJoined then
		TriggerServerEvent('esx_fbijob:spawned')
	end
