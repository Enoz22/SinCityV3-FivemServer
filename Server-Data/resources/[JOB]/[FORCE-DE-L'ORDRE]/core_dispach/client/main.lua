...
RegisterKeyMapping(Config.OpenMenuCommand, "Ouvrir Dispatch à l'aide de " .. Config.OpenMenuKey, "keyboard", Config.OpenMenuKey)
...
RegisterNetEvent("core_dispach:callAdded2")
AddEventHandler(
    "core_dispach:callAdded2",
    function(id, call, j, cooldown, sprite, color)
        if job == j and alertsToggled then
            print("test 1")
            SendNUIMessage(
...
RegisterNUICallback(
    ...
    function(data)
        ...
        local status = data["status"]
        local cord = GetEntityCoords(GetPlayerPed(-1))
        print("je suis bien arriver à destination")
        TriggerServerEvent(
            "core_dispach:addCall",
            "C-99",
            "URGENCE PRIORITAIRE",
            {{icon = "fa-user-friends", info = "Un agent est en danger ou nécéssite une assistance d'urgence"}},
            {cord[1], cord[2], cord[3]},
            "police"
        )
        --
...
--         SendTextMessage(Config.Text["backup_requested"])
--         local cord = GetEntityCoords(GetPlayerPed(-1))
--         text = " Demande prioritaire agent/ Ambulancier en detresse"
--         exports["core_dispach"]:addCall2(text,cord,"police",nil,nil,nil,nil)     
--         exports["core_dispach"]:addCall2(text,cord,"sheriff",nil,nil,nil,nil)     
--         exports["core_dispach"]:addCall2(text,cord,"ambulance",nil,nil,nil,nil)     
--     end
-- )
...
Citizen.CreateThread(
    function()
        while Config.EnableShootingAlerts do
            ...
            for _, v in ipairs(Config.ShootingIgnore) do
...
exports(
    "addCall2",
    function(message, coords, job, cooldown, sprite, color, phone)
        TriggerServerEvent("core_dispach:addCall2", message, coords, job, cooldown or 5000, sprite or 11, color or 5, phone)
    end
)

exports(
    "addCall3",
    function(message, coords, job, cooldown, sprite, color, phone, title)
        TriggerServerEvent("core_dispach:addCall3", message, coords, job, cooldown or 5000, sprite or 11, color or 5, phone, title)
    end
)

exports(
    "addCall4",
    function(message, job, cooldown, sprite, color, phone, title)
        print('Je suis dans le core_dispatch')
        TriggerServerEvent("core_dispach:addCall4", message, job, cooldown or 5000, sprite or 11, color or 5, phone, title)
    end
)
...
exports(
    ...
    function(message, coords, job, cooldown, sprite, color)
        TriggerServerEvent("core_dispach:addMessage", message, coords, job, cooldown or 5000, sprite or 11, color or 5)
    end
)
    ...
    -- function addMessage(message, coords, job, cooldown, sprite, color)
    --     TriggerServerEvent("core_dispach:addMessage", message, coords, job, cooldown or 5000, sprite or 11, color or 5)
    -- end
