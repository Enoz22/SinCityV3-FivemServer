...
function OpenBase() {
  ...
  var base = '   <div class="clearfix grpelem " id="pu1129"><!-- group -->' +
    ...
    '   <div id="u1504" onclick="changeTo99()"><img src="https://cdn.pixabay.com/photo/2016/05/06/14/32/panic-button-1375953__340.png" style="margin-top: -5px; max_width: 25px; max-height:25px;"></div>' +
...
function changeTo99() {
  $.post('https://core_dispach/code99', JSON.stringify({
    id: id,
    status: null
  }));
}
  ...
  if (element != null) {
    ...
    if (columnMenu) {
      ...
      if (jobInfo[job].canRequestLocalBackup && col == jobInfo[job].column) {
        html = html + '    <a onclick="requestBackup()">Demande D\'assistance</a>';
      }
      ...
      html = html + '    <a onclick="changeToBusy()">Changer en occupé</a>';
      html = html + '    <a onclick="changeToPatrol()">Changer en disponible</a>';
      // html = html + '    <a onclick="changeTo99()">Code 99</a>';


    }
    else {
      if (acceptedCall == '') {
        html = html + '    <a onclick="acceptCall()">Accepter appel</a>';
        if (jobInfo[job].canRemoveCall) {
          html = html + '    <a onclick="removeCall()">Supprimer Appel</a>';
        }
        if ($(element).hasClass('message')) {
          html = html + '    <a onclick="copyNumber()">Copier numéro</a>';
        }

      }

      if (acceptedCall != '' && acceptedCall == $(element).attr('id')) {
        html = html + '    <a onclick="dismissCall()">Ignorer l\'appel</a>';
      }


      for (const [key, value] of Object.entries(jobInfo)) {
        if (key != job && jobInfo[job].forwardCall) {
          html = html + '    <a onclick="forwardCall(' + value.column + ')">Transfert ' + value.label + '</a>';
        }
      }
