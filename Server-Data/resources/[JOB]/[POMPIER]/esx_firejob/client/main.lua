...
function OpenFireActionsMenu()
  ...
  ESX.UI.Menu.Open(
    'default', GetCurrentResourceName(), 'onlyalex_feuerwehr_menu',
    {
      title    = "🚒 Pompier 🚒",
      align    = 'right',
      elements = {
        { label = "Mettre / Enlever ça protection respiratoire", value = "pressluftatmung" },
        { label = "Mettre / enlever le casque", value = "helm" },
        { label = "Extincteur d'incendie", value = "feuerloescher" },
        { label = "lampe de poche", value = "taschenlampe" },
        { label = "Hache d'incendie", value = "feuerwehraxt" },
        { label = "Marteau", value = "hammer" },
        { label = "Clé à pipe", value = "zange" },
        --{ label = "La référence", value = "onlyalex" }
      }
    }, 
  function(data, menu)
    ...
    if action == "pressluftatmung" then
      if pa == false then
        exports['progressBars']:startUI(10000, "Vous Mettez une protection respiratoire")
        menu.close()
        Citizen.Wait(10000)
        ESX.ShowNotification("~g~Vous mettez votre kit de protection respiratoire!")
        pa = true
      local playerPed  = GetPlayerPed(-1)
      ...
      z = coords.z - 3
          ...
          onlyflasche = flasche
      AttachEntityToEntity(flasche, playerPed, boneIndex2, -0.3, 0.02, 0.0, 0.0, 90.0, 180.0, true, true, false, true, 1, true)
      ...
      ESX.ShowNotification('Volumen: 3 bar / 1%  ~r~Valeurs limites atteintes!', true)
      Citizen.Wait(20000)
      ...
      local boneIndex  = GetPedBoneIndex(playerPed, 12844)
      local boneIndex2 = GetPedBoneIndex(playerPed, 24818)
      exports['progressBars']:startUI(20000, "Vous Retirer la protection respiratoire")
      ESX.Game.DeleteObject(onlyMaske)
      ESX.Game.DeleteObject(onlyflasche)
      SetPedDiesInWater(playerPed, true)
      ESX.ShowNotification("~r~SCBA supprimé")
      pa = false
      menu.close()
      end
    end
    ...
    if action == "helm" then
      ...
      local boneIndex  = GetPedBoneIndex(playerPed, 12844)
      local boneIndex2 = GetPedBoneIndex(playerPed, 24818)
      exports['progressBars']:startUI(6000, "Mettez le casque")
      Citizen.Wait(5000)
      ...
      ESX.Game.SpawnObject('prop_tool_hardhat', {
        ...
        AttachEntityToEntity(helm, playerPed, boneIndex, 0.0, 0.0, 0.0, 0.0, 90.0, 0.0, true, true, false, true, 1, true)
        ESX.ShowNotification("~g~Casque mis")
        muhHelmet = helm
        helmet = true
        menu.close()
        end)
        ...
        ESX.ShowNotification("~r~Casque retiré!")
        helmet = false
        menu.close()
      end
    end

    if action == "feuerloescher" then
      local playerPed  = GetPlayerPed(-1)
      exports['progressBars']:startUI(5000, "Préparer des extincteurs")
      Citizen.Wait(5000)
      GiveWeaponToPed(
        playerPed --[[ Ped ]], 
        0x060EC506 --[[ Hash ]], 
      ...
      ESX.ShowNotification("~g~Extincteurs prêts à être éteints!")
      menu.close()
    end
    if action == "taschenlampe" then
      local playerPed  = GetPlayerPed(-1)
      exports['progressBars']:startUI(2000, "Sortez la lampe de poche")
      Citizen.Wait(2000)
      GiveWeaponToPed(
        playerPed --[[ Ped ]], 
        0x8BB05FD7 --[[ Hash ]], 
      ...
      ESX.ShowNotification("~g~Lampe de poche prête!")
      menu.close()
    end
    if action == "feuerwehraxt" then
      local playerPed  = GetPlayerPed(-1)
      exports['progressBars']:startUI(6000, "Sortez la hache")
      Citizen.Wait(6000)
      GiveWeaponToPed(
        playerPed --[[ Ped ]], 
        0xF9DCBF2D --[[ Hash ]], 
      ...
      ESX.ShowNotification("~g~Axe fait ressortir!")
      menu.close()
    end
    if action == "hammer" then
      local playerPed  = GetPlayerPed(-1)
      exports['progressBars']:startUI(1000, "Obtenez un marteau")
      Citizen.Wait(1000)
      GiveWeaponToPed(
        playerPed --[[ Ped ]], 
        0x4E875F73 --[[ Hash ]], 
      ...
      ESX.ShowNotification("~g~Sors le marteau!")
      menu.close()
    end
    if action == "zange" then
      local playerPed  = GetPlayerPed(-1)
      exports['progressBars']:startUI(3000, "Sortez la clé à pipe")
      Citizen.Wait(3000)
      GiveWeaponToPed(
        playerPed --[[ Ped ]], 
        0x19044EE0 --[[ Hash ]], 
      ...
      ESX.ShowNotification("~g~Clé à tube retirée!")
      menu.close()
    end
    if action == "onlyalex" then
      ESX.ShowNotification("~b~Fabriqué par POLO | Support on Discord: https://discord.gg/czW6Jqj")
      menu.close()
    end
    ...
    elseif action == "helm" then
      ...
      ESX.Game.SpawnObject('prop_tool_hardhat', {
        ...
        AttachEntityToEntity(helm, playerPed, boneIndex, 0.0, 0.0, 0.0, 0.0, 90.0, 0.0, true, true, false, true, 1, true)
        ESX.ShowNotification("~g~Casque sur")
        muhHelmet = helm
        helmet = true
        menu.close()
        end)
