            ...
            SqlFetch("SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA=@table AND TABLE_NAME=@name",{['@table'] = dbName,['@name']  = (Config and Config.WarehouseTable or "vehicles")},function(r2)  
              ...
              if type(r2) == "table" and r2[1] and r2[1].TABLE_NAME == (Config and Config.WarehouseTable or "vehicles") then
                ...
                SqlFetch("SELECT * FROM "..(Config and Config.ShopTable or "vehicle_shops"),{},function(shopData)
                  ...
                  SqlFetch("SELECT * FROM "..(Config and Config.WarehouseTable or "vehicles").." WHERE category != @model1 and category != @model2 ",{["@model1"] = "svp", ["@model2"] = "gv"},function(vehData)

                    for k,v in pairs(vehData) do

                      table.insert(WarehouseVehicles,{name = v.name,model = v.model,price = v.price,categorie = v.category})
...
CreateShop = function(name,locations,price)
  ...
  SqlExecute("INSERT INTO "..(Config and Config.ShopTable or "vehicle_shops").." SET owner='none',name=@name,type=@type,locations=@locations,employees='{}',stock='{}',displays='{}',funds=0,price=@price",{['@name'] = name, ['@type'] = "{}", ['@locations'] = json.encode(locations),['@price'] = math.max(1,tonumber(price))})
...
VehiclePurchased = function(shopId,vehData,vehId,props)
  ...
  -- else
  --   TriggerClientEvent("esx:showNotification",_source,"Cette catégorie de véhicule ne fait pas partie de ton catalogue")
  -- end
end
...
---------------------------SET JOB VEHICLE------------------------------------
ESX.RegisterServerCallback('vehicleshop:retrieveJobVehicles', function(source, cb, type)
	local xPlayer = ESX.GetPlayerFromId(source)
...
RegisterNetEvent('vehicleshop:setJobVehicleState')
AddEventHandler('vehicleshop:setJobVehicleState', function(plate, state)
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	MySQL.Async.execute('UPDATE owned_vehicles SET `stored` = @stored WHERE plate = @plate AND job = @job', {
		...
		if rowsChanged == 0 then
			print(('[vehicleshop] [^3WARNING^7] %s exploited the garage!'):format(xPlayer.identifier))
		end
	end)
end)


RegisterNetEvent('setjob:concess')
AddEventHandler('setjob:concess', function(source, shop, player)
	local xPlayer = ESX.GetPlayerFromId(player)
  print(json.encode(xPlayer))
  print(source,shop,player)
  if xPlayer.identifier then
    MySQL.Async.execute('UPDATE vehicle_shops SET `owner` = @owner WHERE id = @id ', {
      ['@owner'] = xPlayer.identifier,
      ['@id'] = shop
    }, function(rowsChanged)
      if rowsChanged then
        TriggerClientEvent("esx:showNotification",source,"Le joueur et devenue proprietaire de la concession")
      end
    end)
  else
...
-- ESX.RegisterServerCallback('CheckCategorie', function(Player,cb,vehModel,shopId)
--   local send = 0 
--     MySQL.Async.fetchScalar('SELECT type FROM vehicle_shops WHERE name=@name', {['@name'] = shopId}, function(shop)
--     MySQL.Async.fetchScalar('SELECT category FROM vehicles WHERE model=@model LIMIT 1', {['@model'] = vehModel}, function(veh)
--        for _,v in ipairs(json.decode(shop)) do
--           if veh == v or v == "all"  then
--             send = 1
--           end
--         end
--         if send == 1 then
