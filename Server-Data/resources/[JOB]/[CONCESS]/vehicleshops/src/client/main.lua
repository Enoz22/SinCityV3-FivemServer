...
VehicleShops.WarehouseRefresh = function(data)
  ...
  if InsideWarehouse then
    ESX.ShowNotification("Stock de l'entrepôt rafraîchi. Vous devez rentrer dans le bâtiment.")
  ...
  while true do
    ...
    if InsideWarehouse then
      ...
      if closest and closestDist and closestDist < 5.0 then
        ...
        local closest2,closestDist2 = VehicleShops.GetClosestShop()
        local up = vector3(0.0,0.0,1.0)
        shopName = closest2
        local posA = closest.pos.xyz + up
        DrawText3D(posA.x,posA.y,posA.z + max.z, closest.name.." [$~g~"..closest.price.."~s~]\n[~g~G~s~] Acheter",15.0)
        if IsControlJustPressed(0,47) then
          -- ESX.TriggerServerCallback("CheckCategorie", function(value)
          --   if value == true then
              VehicleShops.PurchaseStock(closest)
            -- elseif value == false then
            --   ESX.ShowNotification("Cette catégorie de véhicule ne fait pas partie de votre entreprise")
            -- end
          -- end,closest.model, shopName)
        end
      end
    else
...
VehicleShops.PurchasedShop = function(shop)
  ...
  ESX.TriggerServerCallback("VehicleShops:PurchaseShop",function(can_buy)
    if can_buy then
      ESX.ShowNotification(string.format("Vous avez acheté la boutique pour $%i.",VehicleShops.Shops[closest].price))
    else
      ESX.ShowNotification("T'as pas d'argents espéce de connard !!!.")
    end
  end,closest)
end
...
VehicleShops.PurchaseStockVehicle = function(vehicle_data,shop_key)
  if VehicleShops.Shops[shop_key].funds >= vehicle_data.price then
    ...
    ESX.ShowNotification("Vous achetez "..label.." pour $"..vehicle_data.price,"success")
    ...
    ESX.TriggerServerCallback("VehicleShops:GenerateNewPlate",function(newPlate)
      ...
      TriggerServerEvent("VehicleShops:VehiclePurchased",shop_key,vehicle_data.name,vehicle_data.key,props)
    ...
    ESX.ShowNotification("Pas assez de fonds.","error")
  end
end
...
VehicleShops.PurchaseStock = function(vehicle)
  ...
  if #elements <= 0 then
    table.insert(elements,{
      label = "Aucun magasin à afficher."
    })
  end
...
VehicleShops.EnterWarehouse = function(...)
  local plyPed = GetPlayerPed(-1)
  ESX.ShowNotification("Spawning shop, veuillez attendre le chargement des modèles.")
  Wait(1000)
...
VehicleShops.ManageDisplays = function(shop_key)
  ...
  if #elements == 0 then
    table.insert(elements,{
      label = "Aucun véhicules à afficher."
    })
  end
...
VehicleShops.ManageDisplayed = function(shop_key)
  ...
  if TableCount(shop.displays) > 0 then
    ...
    table.insert(elements,{
      label = "Aucun Véhicule à afficher."
    })
  end
...
VehicleShops.DoSetPrice = function(shop,vehicle)
  TriggerEvent("Input:Open","Set Price","ESX",function(p)
    ...
    if not price then
      ESX.ShowNotification("Fixez un prix valide.")
      Wait(200)
      VehicleShops.DoSetPrice(shop,vehicle)
    else      
      local vehData = VehicleShops.Shops[shop].displays[vehicle]
      ESX.ShowNotification("Vous fixez le prix du"..(GetLabelText(GetDisplayNameFromVehicleModel(vehData.vehicle.model))).." à $"..price)
...
VehicleShops.ManageShop = function(shop_key)
  local elements = {
    {label = "Ajouter des fonds",value="Add"},
    {label = "Prendre des fonds",value="Take"},
    {label = "Vérifier les fonds",value="Check"},
  }
  ...
  ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'manage_shop', {
    ...
    function(d,m)
      ...
      elseif element.value == "Check" then
        ESX.ShowNotification("Fonds: $"..VehicleShops.Shops[shop_key].funds,1)
...
VehicleShops.ManageVehicles = function(shop_key)
  local clicked = false
  local elements = {
    {label = "Voir les véhicules",value = "Display"},
    {label = "Stocker le Véhicule",value = "Store"},
    {label = "Ajouter le prix du véhicule",value = "Price"},
    {label = "Véhicule de réserve de conduite",value = "Drive"},
  }
        ...
        TriggerEvent("Input:Open","Pay Amount","ESX",function(amount)
          ...
          if not amount then
            ...
            if VehicleShops.Shops[shop_key].funds < amount then
              ESX.ShowNotification("La boutique n'a pas autant de fonds.")
            else
...
VehicleShops.ManageEmployees = function(shop_key)
  local elements = {
    {label = "Embaucher un employé",value = "Hire"},
    {label = "Viré enployer",value = "Fire"},
    {label = "Paye des employer",value = "Pay"},
  }
    ...
    if owner and (VehicleShops.Shops[shop_key].owner:match(owner) or ((VehicleShops.KashId and VehicleShops.KashId..":" or "")..PlayerData.identifier):match(owner)) then
      ...
      if not owner then
        if Config.StockStolenPedVehicles then
          ...
          ESX.ShowNotification("Vous ne pouvez pas stocker les véhicules volés.")
          callback(false)
        end
        return
      else
        if Config.StockStolenPlayerVehicles then
          ...
          ESX.ShowNotification("Vous ne pouvez pas stocker les véhicules des autres joueurs.")
          callback(false)
        end
        return
      end
...
VehicleShops.CreateNew = function(...)
  ...
  if closest and dist and dist < 20.0 then
    ESX.ShowNotification("Vous êtes trop près d'un autre magasin de véhicules.")
    return
  end
  ...
  TriggerEvent("Input:Open","Set Shop Name","ESX",function(n)
    ...
    if not name then ESX.ShowNotification("Entrez un nom valide la prochaine fois."); return; end
    Wait(200)
    TriggerEvent("Input:Open","Set Shop Price","ESX",function(p)
      local price = (p and tonumber(p) and tonumber(p) > 0 and tonumber(p) or false)
      if not price then ESX.ShowNotification("Entrez un prix valide la prochaine fois."); return; end
      while true do
        if not locations.blip then
          if not warnBlip then
            ESX.ShowNotification("Appuyez sur G pour définir l'emplacement du blip.")
            warnBlip = true
          end
          if IsControlJustReleased(0,47) then
        ...
        elseif not locations.entry then
          if not warnEntry then
            ESX.ShowNotification("Appuyez sur G pour définir l\'emplacement du magasin d'entrée / d'achat.")
            warnEntry = true
          end
          if IsControlJustReleased(0,47) then
        ...
        elseif not locations.management then
          if not warnManage then
            ESX.ShowNotification("Appuyez sur G pour définir l'emplacement du menu de gestion.")
            warnManage = true
          end
          if IsControlJustReleased(0,47) then
        ...
        elseif not locations.spawn then
          if not warnSpawn then
            ESX.ShowNotification("Appuyez sur G pour définir l'emplacement d'apparition du véhicule (à l'intérieur).")
            warnSpawn = true
          end
          if IsControlJustReleased(0,47) then
            local plyPed = GetPlayerPed(-1)
        ...
        elseif not locations.purchased then
          if not warnPurchased then
            ESX.ShowNotification("Appuyez sur G pour définir l'emplacement d'apparition du véhicule (à l'extérieur).")
            warnPurchased = true
          end
          if IsControlJustReleased(0,47) then
            local plyPed = GetPlayerPed(-1)
        ...
        elseif not locations.deposit then
          if not warnDeposit then        
            ESX.ShowNotification("Appuyez sur G pour définir l'emplacement de dépôt du véhicule.")
            warnDeposit = true
          end
          if IsControlJustReleased(0,47) then
          ...
          ESX.ShowNotification("Boutique créée, nom: "..name..", Prix: "..price)
