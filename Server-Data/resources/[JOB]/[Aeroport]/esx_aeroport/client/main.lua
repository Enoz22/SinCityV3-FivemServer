...
function IsJobTrue()
    if PlayerData ~= nil then
        ...
        if PlayerData.job ~= nil and PlayerData.job.name == 'aeroport' then
            IsJobTrue = true
        end
        return IsJobTrue
    end
...
function setUniform(job, playerPed)
  TriggerEvent('skinchanger:getSkin', function(skin)
    ...
    if skin.sex == 0 then
      ...
      if job ~= 'citizen_wear' and job ~= 'pilote_outfit' then
        setClipset(playerPed, "") -- "MOVE_F@POSH@" Entre guillemet
      end
    else
...
function OpenCloakroomMenu()
  ...
  local elements = {
    ...
    { label = _U('pilote_outfit'),    value = 'pilote_outfit'},
    { label = _U('pilote_outfit_1'),  value = 'pilote_outfit_1'},
    { label = _U('pilote_outfit_2'),  value = 'pilote_outfit_2'},
    { label = _U('pilote_outfit_3'),  value = 'pilote_outfit_3'}
  }
  ...
  ESX.UI.Menu.Open(
    ...
    function(data, menu)
      ...
      if data.current.value == 'pilote_outfit' then
        setUniform(data.current.value, playerPed)
        isPilote = true
      end

      if
        data.current.value == 'pilote_outfit_1' or
        data.current.value == 'pilote_outfit_2' or
        data.current.value == 'pilote_outfit_3'
      then
        setUniform(data.current.value, playerPed)
      end
    ...
    ESX.TriggerServerCallback('esx_society:getVehiclesInGarage', function(garageVehicles)
      ...
      ESX.UI.Menu.Open(
        ...
        function(data, menu)
          ...
          TriggerServerEvent('esx_society:removeVehicleFromGarage', 'aeroport', vehicleProps)
...
function OpenSocietyActionsMenu()
  ...
  table.insert(elements, {label = _U('Acheter_piece'),    value = 'gps_set2'})
  table.insert(elements, {label = _U('Acheter_fils'),    value = 'gps_set4'})
  if (isPilote or IsGradeBoss()) then
  ...
  ESX.UI.Menu.Open(
    'default', GetCurrentResourceName(), 'aeroport_actions',
    {
      title    = _U('aeroport'),
      align    = 'left',
      elements = elements
    ...
    function(data, menu)
      ...
      if data.current.value == 'gps_set2' then 
        SetNewWaypoint( 178.028, 307.467, 104.392 )
				local source = GetPlayerServerId();
				ESX.ShowNotification("~b~Point ajouté au GPS !")
      end

      if data.current.value == 'gps_set4' then
        SetNewWaypoint( 95.18, -1810.14, 26.095 )
				local source = GetPlayerServerId();
				ESX.ShowNotification("~b~Point ajouté au GPS !")
      end
      ...
      if data.current.value == 'menu_crafting' then
          ...
          ESX.UI.Menu.Open(
              'default', GetCurrentResourceName(), 'menu_crafting',
              {
                  ...
                  align = 'left',
                  elements = {
                      {label = _U('parachute'),     value = 'parachute'}
                      --{label = _U('golem'),         value = 'golem'}
                  }
              },
              function(data2, menu2)
            
                TriggerServerEvent('esx_aeroportjob:craftingParachute', data2.current.value)
    ...
    function(data, menu)
      ...
      if player ~= -1 and distance <= 3.0 then
        ...
        if amount == nil then
            ...
            TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(player), 'society_aeroport', _U('billing'), amount)
        end
...
function OpenGetStocksMenu()
  ...
  ESX.TriggerServerCallback('esx_aeroportjob:getStockItems', function(items)
        ...
        ESX.UI.Menu.Open(
          ...
          function(data2, menu2)
            ...
            if count == nil then
              ...
              TriggerServerEvent('esx_aeroportjob:getStockItem', itemName, count)
            end
...
ESX.TriggerServerCallback('esx_aeroportjob:getPlayerInventory', function(inventory)
        ...
        ESX.UI.Menu.Open(
          ...
          function(data2, menu2)
            ...
            if count == nil then
              ...
              TriggerServerEvent('esx_aeroportjob:putStockItems', itemName, count)
            end
...
function OpenGetFridgeStocksMenu()
  ...
  ESX.TriggerServerCallback('esx_aeroportjob:getFridgeStockItems', function(items)
    ...
    ESX.UI.Menu.Open(
      ...
      {
        title    = _U('aeroport_fridge_stock'),
        elements = elements
      },
      function(data, menu)
        ...
        ESX.UI.Menu.Open(
          ...
          function(data2, menu2)
            ...
            if count == nil then
              ...
              TriggerServerEvent('esx_aeroportjob:getFridgeStockItem', itemName, count)
            end
        ...
        ESX.UI.Menu.Open(
          ...
          function(data2, menu2)
            ...
            if count == nil then
              ...
              TriggerServerEvent('esx_aeroportjob:putFridgeStockItems', itemName, count)
            end
...
function OpenGetWeaponMenu()
  ...
  ESX.TriggerServerCallback('esx_aeroportjob:getVaultWeapons', function(weapons)
    ...
    ESX.UI.Menu.Open(
      ...
      function(data, menu)
        ...
        ESX.TriggerServerCallback('esx_aeroportjob:removeVaultWeapon', function()
          OpenGetWeaponMenu()
...
function OpenPutWeaponMenu()
  ...
  ESX.UI.Menu.Open(
    ...
    function(data, menu)
      ...
      ESX.TriggerServerCallback('esx_aeroportjob:addVaultWeapon', function()
        OpenPutWeaponMenu()
      end, data.current.value)
...
function OpenShopMenu(zone)
    ...
    ESX.UI.Menu.Open(
        'default', GetCurrentResourceName(), 'aeroport_shop',
        {
            title    = _U('shop'),
            elements = elements
        },
        function(data, menu)
            TriggerServerEvent('esx_aeroportjob:buyItem', data.current.value, data.current.price, data.current.realLabel)
        end,
        function(data, menu)
            menu.close()
        end
...
AddEventHandler('esx_aeroportjob:hasEnteredMarker', function(zone)
...
AddEventHandler('esx_aeroportjob:hasExitedMarker', function(zone)
    ...
    while true do
        ...
        if IsJobTrue() then
            ...
            if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
                ...
                TriggerEvent('esx_aeroportjob:hasEnteredMarker', currentZone)
            end

            if not isInMarker and HasAlreadyEnteredMarker then
                HasAlreadyEnteredMarker = false
                TriggerEvent('esx_aeroportjob:hasExitedMarker', LastZone)
            end
...
Citizen.CreateThread(function()
  while true do
      ...
      if IsControlJustReleased(0,  Keys['E']) and IsJobTrue() then
        ...
        if CurrentAction == 'delete_vehicle' then
          ...
          if Config.EnableSocietyOwnedVehicles then
            ...
            TriggerServerEvent('esx_society:putVehicleInGarage', 'aeroport', vehicleProps)
            ...
            then
              TriggerServerEvent('esx_service:disableService', 'aeroport')
            end
        ...
        if CurrentAction == 'menu_boss_actions' and IsGradeBoss() then
          ...
          TriggerEvent('esx_society:openBossMenu', 'aeroport', function(data, menu)
    ...
    if IsControlJustReleased(0,  Keys['F6']) and IsJobTrue() and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'aeroport_actions') then
