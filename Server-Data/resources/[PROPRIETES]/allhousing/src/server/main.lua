...
GetInventory = function(source,cb,house)
  ...
  if Config.Debug == true then
    print(v.Inventory.Cash)
    print(v.Inventory.DirtyMoney)
    print(json.encode(v.Inventory.Items))
    print(json.encode(v.Inventory.Weapons))
  end
end
...
PurchaseHouse = function(house)
  ...
  if not v.Owned then
    ...
    if afford then
        ...
        if salePrice > 0 then
          ...
          if v.ResaleJob and v.ResaleJob:len() > 1 then
            ...
            if societyAmount and societyAccount then
              ...
              -- AddSocietyMoney('society_avocat',0.9 * v.Price)
            end
          end
        end
      end
      ...
      SqlExecute("INSERT allhousinglog SET time=@time,acheteur=@acheteur,prix=@price,houseid=@houseid",{['@acheteur'] = v.OwnerName ,['@time'] = os.date(),['@price'] = v.Price,['@houseid'] = v.Id})
      ...
      if v.Owner and v.Owner:len() >= 1 then
        ...
        if salePrice > 0 then
          ...
          if v.ResaleJob and v.ResaleJob:len() > 1 then
            ...
            local societyAccount = Config.CreationJobs[v.ResaleJob].account
            local societyAccount2 = GetSociety("society_avocat")
            if societyAmount and societyAccount then
              AddSocietyMoney(societyAccount,societyAmount)
