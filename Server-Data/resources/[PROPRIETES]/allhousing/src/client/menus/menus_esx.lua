  ...
  if IsPedInAnyVehicle(ped,false) then
    ...
    if canStore then
      ...
      if doStore then
       ...
       -- DeleteVehicle(veh)  
       -- TriggerServerEvent("Allhousing:VehicleStored",d.Id,props.plate,props)        -- Enregistrement vheicule par maison

        
        local vehFuel = GetVehicleFuelLevel(veh)
        

        --ESX.TriggerServerCallback('t1ger_garage:storeVehicle',function(ped, cb, d.Id, props, vehFuel)
         
          ESX.TriggerServerCallback('t1ger_garage:storeVehicle',function(ownedVehicle)
            if ownedVehicle then
              DeleteVehicle(veh)
              -- TriggerEvent('t1ger_garage:setState2', props.plate)
              ShowNotification(Labels["VehicleStored"])
              FreezeEntityPosition(GetPlayerPed(-1), unfreeze )
            else
              ShowNotification(Labels["CantStoreVehicle"])
              FreezeEntityPosition(GetPlayerPed(-1), unfreeze )
            end
          end, d.Id, props, vehFuel)
   ...
   -- FreezeEntityPosition(GetPlayerPed(-1),false)
    ...
    TriggerEvent('t1ger_garage:menugethouse',d) 
    FreezeEntityPosition(GetPlayerPed(-1), unfreeze )
   ...
   --[[ local elements = {}
    ...
    ESX.UI.Menu.Open('default', GetCurrentResourceName(), "entry_owner_menu",{
      ...
      (#vehicles > 0 and function(submitData,submitMenu)
        --TriggerServerEvent('Allhousing:VehicleSpawned',submitData.current.value.vehicle.plate)        -- Enre gistrement spawn 
        --SpawnVehicle(d.Garage,submitData.current.value.vehicle.model,submitData.current.value.vehicle)
