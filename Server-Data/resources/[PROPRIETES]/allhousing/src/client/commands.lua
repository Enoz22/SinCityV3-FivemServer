    ...
    exports["input"]:Open("Set Sale Price",(Config.UsingESX and Config.UsingESXMenu and "ESX" or "Native"),function(data)
      ...
      if Config.UseMLO then
        ...
        if Config.UsingESX and Config.UsingESXMenu then
          local elements = {}
          local greenlist = {}
          for k,v in pairs(ShellModels2) do
          ...
          ESX.UI.Menu.Open('default', GetCurrentResourceName(), "create_house_shell",{
            ...
            function(submitData,submitMenu)
              if submitData.current.label == "Tier1" or submitData.current.label == "Tier2" or submitData.current.label == "Tier3" or submitData.current.label == "Tier4" or submitData.current.label == "Tier5" or submitData.current.label == "Business" or submitData.current.label == "Stockage" or submitData.current.label == "Magasin" then
              else
                is_shell = submitData.current.label
                submitMenu.close()
              end
      ...
      if is_shell then
        ...
        if Config.UsingESX and Config.UsingESXMenu then
          ...
          local shell = CreateNativeUIMenu(Label['AvailableShells'],"")
          for k,v in pairs(ShellModels) do
            shells[k] = false
