  ...
  while true do
    if not robbery.inHouse then
      ...
      if closestDist < Config.ActDist then
        if robbery.cops and robbery.cops >= Config.MinCopsOnline and not robbery.tryHouse then
          if Config.UseDraw3D then
            ESX.Game.Utils.DrawText3D(closest, "Appuyer sur [~r~E~s~] pour forcer la porte.",1,7)
          else
            ESX.ShowHelpNotification("Appuyer sur ~INPUT_PICKUP~ pour forcer la porte.")
          end
      ...
      if exitDist and exitDist < Config.ActDist then
        if Config.UseDraw3D then
          ESX.Game.Utils.DrawText3D(robbery.inHouse.exit.xyz + vector3(0.0,0.0,1.0), "Appuyer sur [~r~E~s~] pour sortir.",1,7)
        else
          ESX.ShowHelpNotification("Appuyer sur ~INPUT_PICKUP~ pour sortir.")
        end
        if IsControlJustReleased(0, 38) then
          robbery.doLeave()
        end
      ...
      if robbery.plyData.job.name ~= Config.PoliceJob then
        ...
        if closestDist < Config.ActDist then
          if Config.UseDraw3D then
            ...
            ESX.Game.Utils.DrawText3D(pos, "Appuyer sur [~r~E~s~] pour essayer de voller "..robbery.currentInterior.loot[closest].tab..".",1,7)
          else
            ESX.ShowHelpNotification("Appuyer sur ~INPUT_PICKUP~ pour essayer de voller "..robbery.currentInterior.loot[closest].tab..".")
          end
  ...
  if robbery.inHouse then
    ...
    elseif robbery.soundLevel > 80.0 then
      if not robbery.lastWarn or ((GetGameTimer() - robbery.lastWarn) > 5000) then
        ...
        ESX.ShowNotification("~r~Tu fais trop de bruit !~s~")
      end
      ...
      if job and job.name == Config.PoliceJob then
          ...
          for key,val in pairs(inv) do
            if val.name == Config.LockpickItemName then
              if val.count and val.count > 0 then 
                ...
                if val.count and val.count <= 0 then
                  ESX.ShowNotification("Tu n'as pas de crochet")
                  return
                end
              end
            end
        ...
        if found or not Config.NeedLockpick then
          ...
          if Config.MinigameForEntry then
            ...
            TriggerEvent('lockpicking:StartMinigame', data.difficulty, 'robbery:lockpickResult')
          else
            robbery.currentInterior = data
...
robbery.lockpickResult = function(res)
  ...
  if res then
    ...
    ESX.ShowNotification("La police à été alerté.")
...
robbery.alertPolice = function(pos)
  ...
  if robbery.plyData.job.name == Config.PoliceJob then
    Citizen.CreateThread(function(...)
      ESX.ShowNotification("Une personne essaye de braquer une maison! [~r~H~s~]")
