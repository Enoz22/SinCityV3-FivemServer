...
SpawnLocalVehicle = function(data)
	...
	if not esx.Game.IsSpawnPointClear(spawnpoint["position"], 3.0) then 
		TriggerEvent('notification', 'Déplacez le véhicule de la route.', 1)
...
SpawnVehicle = function(data,recuperar)
	...
	if not esx.Game.IsSpawnPointClear(spawnpoint["position"], 3.0) then 
		TriggerEvent('notification', 'Déplacez le véhicule de la route', 2)
	...
	for i = 1, #gameVehicles do
        ...
        if DoesEntityExist(vehicle) then
			if Config.Trim(GetVehicleNumberPlateText(vehicle)) == Config.Trim(vehicleProps["plate"]) then
                TriggerEvent('notification', 'Vôtre véhicule n`\' est pas dans un garage.', 2)
    ...
    if recuperar then
        TriggerEvent('notification', 'Vous venez de payer 200$ pour récupérer vôtre véhicule !', 1)
    ...
    if DoesEntityExist(vehicle) then
		...
		esx.TriggerServerCallback("erp_garage:validateVehicle", function(valid)
			if valid then
                ...
                CloseMenu()
                TriggerEvent('notification', 'Ce n\'est pas votre véhicule.', 2)
			end
...
SaveInGarage = function(garage)
    ...
    TriggerEvent('notification', 'Vôtre véhicule est stocké dans le garage '..garage, 3)
    Citizen.Wait(500)
...
DrawScriptMarker = function(markerData)
    ...
    esx.ShowHelpNotification("~g~E  ~w~ pour consulter la borne de stationnement")
end
