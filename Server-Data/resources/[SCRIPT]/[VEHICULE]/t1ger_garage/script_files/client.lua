...
function OpenVehicleInfoDisplay(info,garage)
	...
	--	{ label = "Fuel: "..round(info.fuel,1).."% | Engine: "..round(info.vehicle.engine,2), value = nil, fuel = info.fuel, engine = info.vehicle.engine },
	}
...
Citizen.CreateThread(function()
    while true do
		...
		for k,v in pairs(Config.JobGarages) do
			local distance = GetDistanceBetweenCoords(coords.x, coords.y, coords.z, v.pos[1], v.pos[2], v.pos[3], false)
			if jobGarage ~= nil then
				distance = GetDistanceBetweenCoords(coords.x, coords.y, coords.z, jobGarage.pos[1], jobGarage.pos[2], jobGarage.pos[3], false)
				while jobGarage ~= nil and distance > 2.0 do
					jobGarage = nil
					Citizen.Wait(1)
				end
				if jobGarage == nil then
...
function OpenJobGarageMenu(v)
	local playerPed  = PlayerPedId()
	local elements = {
		{ label = Lang['get_veh_from_garage'], value = "get_vehicle" },
		{ label = Lang['store_veh_to_garage'], value = "store_vehicle" },
	}
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), "job_garage_main_menu",
		{
			title    = "Garage: "..tostring(v.job),
			align    = "center",
			elements = elements
		},
	function(data, menu)
		if(data.current.value == 'get_vehicle') then
			OpenJobGarageVehList(v)
			menu.close()
			jobGarage = nil
		end
		if(data.current.value == 'store_vehicle') then
...
function OpenJobGarageVehList(v)
	local playerPed = PlayerPedId()
	local elements = {}
	for k,j in pairs(v.vehicles) do
		local vehName 	= GetDisplayNameFromVehicleModel(j)
		local vehLabel 	= GetLabelText(vehName)
		print("vehName: "..vehName.." | model: "..j.." vehLabel: "..vehLabel)
		table.insert(elements,{label = vehLabel, model = j, value = v})
	end
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), "job_garage_vehicle_list",
		{
			title    = "Garage : "..tostring(v.job),
			align    = "center",
			elements = elements
	...
	function(data, menu)
		SpawnJobVehicle(data.current.model, data.current.value)
		menu.close()
		garage = nil
	end, function(data, menu)
		menu.close()
...
function SpawnJobVehicle(model, val)
	ESX.Game.SpawnVehicle(model,{x = val.pos[1], y = val.pos[2], z = val.pos[3] + 1}, val.heading, function(jobVeh)
		local jobPlate = exports['t1ger_cardealer']:ProduceNumberPlate()
		print(jobPlate)
		SetVehRadioStation(jobVeh, "OFF")
		SetVehicleNumberPlateText(jobVeh, jobPlate)
		SetPedIntoVehicle(PlayerPedId(),jobVeh,-1)
		if Config.HasFuelEvent then
			TriggerEvent("fuel:setFuel", jobVeh, 75.0)
		else
			SetVehicleFuelLevel(jobVeh, 75.0)
		end
		TriggerServerEvent('t1ger_keys:addTempKeysToVeh',jobPlate)
	end)
end
		...
		for k,v in pairs(SpawnedCars) do
			...
			if distance <= 2.5 then
				if GetEntityModel(GetVehiclePedIsIn(PlayerPedId())) == v.vehicle.model then
					Draw3DText(v.pos[1], v.pos[2], v.pos[3]-0.2, "~r~[E]~s~ | Sortir le vehicule")
					Draw3DText(v.pos[1], v.pos[2], v.pos[3]-0.3, "Essence: "..round(v.fuel,1).."% | Moteur: "..round(v.vehicle.engine,2))
					if IsControlJustPressed(0, 38) then
