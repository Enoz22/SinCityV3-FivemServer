...
ESX.RegisterServerCallback('t1ger_garage:fetchImpoundedVehicles', function(source, cb, label)
	...
	MySQL.Async.fetchAll("SELECT * FROM owned_vehicles WHERE owner=@identifier AND garage = @garage",{['@identifier'] = xPlayer.getIdentifier(), ['@garage'] = label}, function(results) 
		for k,v in pairs(results) do
			local vehicle = json.decode(v.vehicle)
			table.insert(impoundVehList,{vehicle = vehicle, plate = v.plate, fuel = v.fuel})
		end
		cb(impoundVehList)
	end)
end)
...
ESX.RegisterServerCallback('t1ger_garage:fetchSeizedVehicles', function(source, cb, label)
	...
	MySQL.Async.fetchAll("SELECT * FROM owned_vehicles WHERE garage = @garage",{['@garage'] = label}, function(results) 
		for k,v in pairs(results) do
			local vehicle = json.decode(v.vehicle)
			table.insert(seizedVehList,{vehicle = vehicle, plate = v.plate, fuel = v.fuel})
		end
		cb(seizedVehList)
	end)
end)

ESX.RegisterServerCallback('t1ger_garage:SocietyVehicles', function(source, cb, job)
	local xPlayer = ESX.GetPlayerFromId(source)
	local socVehList = {}

	MySQL.Async.fetchAll("SELECT * FROM owned_vehicles WHERE job = @job",{['@job'] = job}, function(results) 
		for k,v in pairs(results) do
			local vehicle = json.decode(v.vehicle)
			table.insert(socVehList,{vehicle = vehicle, plate = v.plate, fuel = v.fuel})
		end
		cb(socVehList)
	end)
end)
    ...
    MySQL.Async.fetchAll("SELECT * FROM owned_vehicles WHERE owner = @owner and plate = @plate",{['@owner'] = xPlayer.getIdentifier(), ['@plate'] = plate}, function(foundVeh)
        if #foundVeh > 0 then
            for k,v in pairs(foundVeh) do
                MySQL.Sync.execute("UPDATE owned_vehicles SET state = @state, vehicle = @vehicle, garage = @garage, fuel = @fuel WHERE owner = @identifier AND plate = @plate", {
                    ['@vehicle'] = vehicle,
...
ESX.RegisterServerCallback('t1ger_garage:storeVehicle2',function(source, cb, label, vehProps, vehFuel)
	local xPlayer = ESX.GetPlayerFromId(source)
    local plate, fuel, vehicle = vehProps.plate, vehFuel, json.encode(vehProps)
    MySQL.Async.fetchAll("SELECT * FROM owned_vehicles WHERE job = @job and plate = @plate",{['@job'] = xPlayer.job.name, ['@plate'] = plate}, function(foundVeh)
        if #foundVeh > 0 then
            for k,v in pairs(foundVeh) do
                MySQL.Sync.execute("UPDATE owned_vehicles SET state = @state, vehicle = @vehicle, garage = @garage, fuel = @fuel WHERE job = @job AND plate = @plate", {
                    ['@vehicle'] = vehicle,
...
RegisterServerEvent('t1ger_garage:setState')
AddEventHandler('t1ger_garage:setState', function(plate)
    print("State"..plate)
...
RegisterServerEvent('t1ger_garage:setState2')
AddEventHandler('t1ger_garage:setState2', function(plate)
    print("State2"..plate)
                MySQL.Sync.execute("UPDATE owned_vehicles SET state = @state WHERE plate = @plate", {
                    ['@plate'] = plate,
                    ['@state'] = 1
