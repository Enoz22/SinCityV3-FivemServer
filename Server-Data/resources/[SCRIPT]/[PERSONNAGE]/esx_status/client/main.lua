...
AddEventHandler('esx_status:registerStatus', function(name, default, color, visible, tickCallback)
	local status = CreateStatus(name, default, color, visible, tickCallback, false)
	...
	for i=1, #Status, 1 do
		for j=1, #status, 1 do
			if Status[i].name == status[j].name then
				...
				Status[i].setAddict(status[j].addiction)
			end
		end
	end
...
RegisterNetEvent('esx_status:addict')
AddEventHandler('esx_status:addict', function(name, addictState)
	for i=1, #Status, 1 do
		if Status[i].name == name then
			Status[i].setAddict(addictState)
			break
		end
	end
