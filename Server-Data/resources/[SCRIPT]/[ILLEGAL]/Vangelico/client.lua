...
function getHeal(p)
		...
		local heal = GetEntityHealth(p)
			BeginTextCommandScaleformString("STRING")
			AddTextComponentScaleform(heal)
...
RegisterNetEvent('av_vangelico:getMask') -- get items
AddEventHandler('av_vangelico:getMask', function(source, value)
				...
				for k,v in pairs(skin) do
					...
					if k == "mask_1"  then
						...
						if v == 0 then
							print("damage")
							getHeal(p)
							ApplyDamageToPed(source,value, false)
						end
					end
				end
		end)
...
-- 	while true do
-- 		Wait(1)
-- 		for k in pairs(Config.ShootZone) do
-- 			local distance = GetDistanceBetweenCoords(coords, Config.ShootZone[k].x, Config.ShootZone[k].y, Config.ShootZone[k].z, true)
-- 				if distance <= 10 then
...
RegisterNetEvent('av_vangelico:bombaFx') -- get items
...
AddEventHandler('av_vangelico:humo', function()
	if #(GetEntityCoords(PlayerPedId()) - vector3(-632.39, -238.26, 38.07)) < 300 then
		...
		while true do
			TriggerEvent("av_vangelico:getMask", 20) -- apply 5 to damage to ped 
			cuenta = cuenta + 1			
