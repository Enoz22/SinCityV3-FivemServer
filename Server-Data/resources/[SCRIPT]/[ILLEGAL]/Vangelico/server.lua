...
ESX.RegisterServerCallback('av_vangelico:cooldown', function(source,cb)
	...
	local xPlayer  = ESX.GetPlayerFromId(source)
	local cops = 0
	local thermite = xPlayer.getInventoryItem('thermite').count
	...
	if thermite >= 1 then
		...
		cb(true)
		TriggerClientEvent('esx:showNotification', source, "Vous devais avoir une thermite pour cela")
	end
end)


--[[ESX.RegisterServerCallback('av_vangelico:getItem', function(source,cb)
 	local xPlayer  = ESX.GetPlayerFromId(source)
