			...
			if Config.EnableOpeningHours then
				...
				if Config.CloseHour < Config.OpenHour then
					if (ClockTime >= Config.OpenHour and ClockTime < 24) or (ClockTime <= Config.CloseHour -1 and ClockTime > 0) then
						...
						Draw3DText(Config.PawnshopLocation.x, Config.PawnshopLocation.y, Config.PawnshopLocation.z, "Reviens la nuit, le recelleur ouvre à ~r~" .. Config.OpenHour ..":00")
					end
				else
					if ClockTime >= Config.OpenHour and ClockTime <= Config.CloseHour - 1 then
						if not menuOpen then
							Draw3DText(Config.PawnshopLocation.x, Config.PawnshopLocation.y, Config.PawnshopLocation.z, "~g~E~w~ - Le recelleur est disponible")
							if IsControlJustReleased(0, 38) then
								wasOpen = true
								OpenPawnshop()
							end
						...
						Draw3DText(Config.PawnshopLocation.x, Config.PawnshopLocation.y, Config.PawnshopLocation.z, "Le recelleur est fermé, reviens la nuit après ~r~" .. Config.OpenHour ..":00")
					end
				end
			else
				if not menuOpen then
