...
BeginLockpick = function(id, data)
    ...
    while true do
        if IsEntityPlayingAnim(PlayerPedId(), 'missheistfbisetup1', 'hassle_intro_loop_f', 3) then
            ...
            ESX.ShowNotification("Tu as annulé le crochetage...")
            break
        end
        drawTxt(0.97, 0.6, 1.0, 1.0, 0.5, "Lockpicking... ~b~" .. math.floor((timer / 1000) / Config.LockPickTime * 100) .. "~s~%", 255, 255, 255, 255)
        Wait(0)
    end
end
                    ...
                    if dstcheck <= 1.3 then
                        ...
                        if IsControlJustReleased(0, 38) then
                            if not cachedData["houseData"][id]["Lootables"][LootablesId] then
                                ...
                                ESX.ShowNotification("Quelqu'un a déjà récupéré le butin.")
                            end
                        end
                    end
...
LootPlace = function(data, id, LootablesId, LootablesData)
    ...
    while GetGameTimer() - cachedData["startedSearching"] < Config.SearchTime do
        ...
        drawTxt(0.97, 0.6, 1.0, 1.0, 0.5, " En train de chercher..." ..LootablesId, 255, 255, 255, 255)
    end
    ...
    ESX.TriggerServerCallback("burglary:lootItem", function(loot)
        if loot then
            ESX.ShowNotification("tu as trouvé " ..loot)
        end
    end)
...
CameraMenu = function()
    ...
    ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_warehouse',
        {
            ["title"]    = 'Camera Control Center',
            ["align"]    = 'center',
    ...
    Citizen.CreateThread(function()
        ...
        while cachedData["cameraMode"] do
            ...
            elseif IsControlJustReleased(0, 177) then
                DrawBusySpinner("Disconnecting from security cameras...")
                DoScreenFadeOut(1500)
                Wait(2500)
                ExitInstance()
