...
AddEventHandler('loffe_robbery:pickUp', function(store)
    ...
    local randomAmount = math.random(Config.Shops[store].black_money[1], Config.Shops[store].black_money[2])
    xPlayer.addBlack_Money(randomAmount)
...
ESX.RegisterServerCallback('loffe_robbery:canRob', function(source, cb, store)
    ...
    for i = 1, #xPlayers do
        ...
        if xPlayer.job.name == 'police' or xPlayer.job.name == 'sheriff' or xPlayer.job2.name == 'sheriff' then
            cops = cops + 1
        end
    end
