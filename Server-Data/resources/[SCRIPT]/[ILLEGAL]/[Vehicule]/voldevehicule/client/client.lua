...
local JobPED = nil
RegisterNetEvent("t1ger_carthief:spawnNPC")
AddEventHandler("t1ger_carthief:spawnNPC",function(NPC)
...
function GetJobFromNPC()
    ...
    ESX.TriggerServerCallback('t1ger_carthief:getJobCooldown', function(cooldown)
    ...
    ESX.UI.Menu.Open('default', GetCurrentResourceName(), "choose_risk_grade_menu",
        ...
        function(data, menu)
            ...
            if data.current.value == "cancel_interaction_with_npc" then
                ...
                TriggerServerEvent("t1ger_carthief:GetRiskGrade",data.current.value, data.current.BuyPrice, data.current.MinCops, data.current.Cars )
                Citizen.Wait(100)
                menu.close()
                ClearPedTasks(player)
                FreezeEntityPosition(player,false)
...
RegisterNetEvent("t1ger_carthief:BrowseAvailableJobs")
AddEventHandler("t1ger_carthief:BrowseAvailableJobs",function(spot,grade,car)
    ...
    if currentID == 100 then
        ...
        SelectedID = id
        TriggerEvent("t1ger_carthief:StartTheJob",id,grade,car)
    end
end)

-- Event to browse through available locations:
RegisterNetEvent("t1ger_carthief:StartTheJob")
AddEventHandler("t1ger_carthief:StartTheJob",function(id,grade,car)
    ...
    TriggerServerEvent("t1ger_carthief:syncJobsData",Config.CarJobs)
    Citizen.Wait(500)
    ...
    while not JobCompleted do
        ...
        if Config.CarJobs[id].InProgress == true then
            ...
            if (GetDistanceBetweenCoords(coords, CurrentJob.Spot[1], CurrentJob.Spot[2], CurrentJob.Spot[3], true) < 150) and not CurrentJob.CarSpawned then
                ...
                ESX.Game.SpawnVehicle(VehHash, VehCoords, CurrentJob.Heading, function(vehicle)
                ...
                if GetEntityModel(lockpickedVehicle) == VehHash then
                    ...
                    if (GetDistanceBetweenCoords(coords, Deliver2.Pos[1], Deliver2.Pos[2], Deliver2.Pos[3], true) < 2.0) and not carIsDelivered then
                        ...
                        if IsControlJustPressed(0, Config.KeyToDeliver) then
                            ...
                            TriggerServerEvent("t1ger_carthief:JobComplete",VehPrice,RoundHealth)
...
RegisterNetEvent("t1ger_carthief:syncJobsData")
