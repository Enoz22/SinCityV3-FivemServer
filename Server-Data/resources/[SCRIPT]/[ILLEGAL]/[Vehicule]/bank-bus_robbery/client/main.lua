local MiejsceMisji =  {x = 960.71197509766, y = -215.51979064941, z = 76.2552947998} --<< où se trouve le marqueur de mission
local dealerCoords =  {x = 960.78, y = -216.25, z = 76.25}  							--<< miejsce gdzie stoi dealer NPC
local pojazdCoords1 = {x = -1327.479736328, y = -86.045326232910, z = 49.31}  		--<< Vous trouverez ci-dessous les coordonnées pour la reproduction aléatoire des véhicules
...
local BronKierowca = "WEAPON_SMG" 		--Arme du conducteur
local BronPasazer = "WEAPON_SMG"  			--Arme du passager
local CzasDetonacji = 20 * 1000 			--Temps de la detonation
local czasPakowania = 30 * 1000 			--Temps pour prendre l'argent apres l'explosion
...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(0)
            ...
            if dist <= 1.0 then
				DrawText3D(MiejsceMisji.x, MiejsceMisji.y, MiejsceMisji.z, "Appuyez sur ~g~[E]~b~ accepter une mission pour 5 000$")
...
AddEventHandler('napadtransport:infodlalspd', function(x, y, z)	
	if PlayerData.job ~= nil and PlayerData.job.name == 'police' then
        ESX.ShowNotification('~y~Un convoi se fait devaliser !~n~~g~Regarde ton GPS.')
        ...
        AddTextComponentString('Attack Transport')
...
AddTextComponentString("Votre cible est marquée sur la carte, alors préparez-vous pour que la mission réussisse.")
Citizen.InvokeNative(0x1E6611149DB3DB6B, "CHAR_DETONATEBOMB", "CHAR_DETONATEBOMB", true, 1, "Vol de convoi", "~o~Mission", czas)
...
BeginTextCommandSetBlipName("STRING")
AddTextComponentString('Nakit ile Kamyon')
    ...
    while true do
		...
		if zrespione == 1 then
			...
			DrawMarker(0, transCoords.x, transCoords.y, transCoords.z+4.5, 0, 0, 0, 0, 0, 0, 1.0, 1.0, 1.0, 135, 31, 35, 100, 1, 0, 0, 0)
				...
				ostrzezenie = 1
				ESX.ShowNotification('~y~Débarrassez-vous des gardes avant de placer une charge.')
				end
			else
			Citizen.Wait(500)
			end
			...
			if dist <= 4.5 and wysadzony == 0 then
				hintToDisplay('Pour faire sauter la porte arrière et obtenir les billets appuyez sur ~INPUT_DETONATE~ !')
...
if IsVehicleStopped(transport) then
	...
	ESX.ShowNotification('~y~Bombe placer elle explose dans ~g~'..CzasDetonacji / 1000 ..' secondes.')
	...
	wyjebalo = 1
	ESX.ShowNotification('~y~Commencer à recuperer l\'argent.')
	RemoveBlip(oznaczenie)
	PowiadomPsy()
	else
	ESX.ShowNotification('~y~Le véhicule doit être vide pour accueillir la charge')
	end
else
ESX.ShowNotification('~y~Le véhicule doit être vide pour accueillir la charge')
end
...
Citizen.CreateThread(function()
    while true do
		...
		if wyjebalo == 1 then
			...
			if dist <= 4.5 then
				hintToDisplay('Recuperez l\'argent ~INPUT_PICKUP~ ')
				if IsControlJustPressed(0, Keys['E']) then 
				wyjebalo = 0
				ZabieranieSiana()
				Citizen.Wait(500)
...
FreezeEntityPosition(GetPlayerPed(-1), true)
ESX.ShowNotification('~y~Vous recuperez l\'argent.')
