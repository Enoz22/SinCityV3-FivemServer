...
function makeToast(target, subject, message)
    TriggerClientEvent('esx_uteknarkCoke:make_toast', target, subject, message)
end
...
function antispam(ident)
	datauke = false
	MySQL.Async.fetchAll("SELECT player FROM uteknark_Coke WHERE player=@player;", {['@player']=ident}, function(results)
				local getN = 0
		...
		if HappyHourHarvest(source) then
			...
			Wait(200)
				...
				if datauke2 then
					...
					local hits = cropstateCoke.octree:searchSphere(location, Config.Distance.Space)
					if #hits > 0 then
						return false
					end
					
					verbose('Planting at',location,'in soil', soil)
					cropstateCoke:plant(location, soil, 1, xPlayer.identifier)
					return true
				else
...
ESX.RegisterUsableItem('grainecoka', function(source)
	TriggerClientEvent('esx_uteknarkCoke:attempt_plant', source)
end)

function doScenario(who, what, where)
    verbose('Telling', who,'to',what,'at',where)
    TriggerClientEvent('esx_uteknarkCoke:do', who, what, where)
end

--RegisterNetEvent('esx_uteknarkCoke:localisestart')
ESX.RegisterServerCallback('esx_uteknarkCoke:localisestart', function(playerId, cb)
    cb(LocationRandomizer)
end)



RegisterNetEvent('esx_uteknarkCoke:success_plant')
AddEventHandler ('esx_uteknarkCoke:success_plant', function(location, soil)
    local src = source
...
RegisterNetEvent('esx_uteknarkCoke:log')
AddEventHandler ('esx_uteknarkCoke:log',function(...)
    local src = source
    log(src,GetPlayerName(src),...)
end)

RegisterNetEvent('esx_uteknarkCoke:test_forest')
AddEventHandler ('esx_uteknarkCoke:test_forest',function(forest)
    local src = source
    ...
    if IsPlayerAceAllowed(src, 'command.uteknark') then
        ...
        for i, tree in ipairs(forest) do
            cropstateCoke:plant(tree.location, soil, tree.stage)
            if i % 25 == 0 then
                Citizen.Wait(0)
            end
        end
...
Citizen.CreateThread(function()
    ...
    while not databaseReady do
        ...
        if state == "started" then
            Citizen.Wait(1000)
            cropstateCoke:load(function(plantCount)
    ...
    while true do
        ...
        for id, plant in pairs(cropstateCoke.index) do
                ...
                if stageData.interact then
					...
					if now >= relevantTime then
                        if plant.data.stage < #GrowthOpium then
                            ...
                            cropstateCoke:update(id, plant.data.stage + 1)
                        else
...
local commands = {
            ...
            if cropstateCoke.index[plant] then
                if args[2] and string.match(args[2], "^%d+$") then
                    ...
                    if stage > 0 and stage <= #GrowthOpium then
                        ...
                        cropstateCoke:update(plant, stage)
                    else
    ...
    forest = function(source, args)
        if source == 0 then
            ...
            TriggerClientEvent('esx_uteknarkCoke:test_forest', source, count, randomStage)

        end
    end,
    stats = function(source, args)
        if cropstateCoke.loaded then
            local totalTime = 0
    ...
    groundmat = function(source, args)
        if source == 0 then
            ...
            TriggerClientEvent('esx_uteknarkCoke:groundmat', source)
        end
    end,
    pyro = function(source, args)
        if source == 0 then
            ...
            TriggerClientEvent('esx_uteknarkCoke:pyromaniac', source)
        end
    end,
}
