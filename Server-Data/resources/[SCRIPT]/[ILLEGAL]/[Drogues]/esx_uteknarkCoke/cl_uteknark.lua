...
function interactHelp(stage,action)
    ...
    AddTextComponentInteger(#GrowthCoke)
...
function serverlog(...)
    TriggerServerEvent('esx_uteknarkCoke:log',...)
end

RegisterNetEvent('esx_uteknarkCoke:make_toast')
AddEventHandler ('esx_uteknarkCoke:make_toast', function(subject,message)
        ...
        if Config.Soil[material] then
            ...
            if flatEnough(surfaceNormal) then
                ...
                if plantDistance <= Config.Distance.Interact then
                    local hits = cropstateCoke.octree:searchSphere(hitLocation, Config.Distance.Space)
                    if #hits > 0 then
...
RegisterNetEvent('esx_uteknarkCoke:do')
AddEventHandler ('esx_uteknarkCoke:do', function(scenarioName, location)
...
RegisterNetEvent('esx_uteknarkCoke:attempt_plant')
AddEventHandler ('esx_uteknarkCoke:attempt_plant', function()
    local plantable, message, location, _, soil = getPlantingLocation()
    if plantable then
        TriggerServerEvent('esx_uteknarkCoke:success_plant', location, soil)
        ...
        if #activePlants > 0 then
            ...
            if closestDistance and not IsPedInAnyVehicle(playerPed) then
                ...
                if closestDistance <= Config.Distance.Interact then
                    local stage = GrowthCoke[closestPlant.stage]
                    ...
                    if now >= lastAction + Config.ActionTime then
                        if IsDisabledControlJustPressed(0, 44) then
                            ...
                            TriggerServerEvent('esx_uteknarkCoke:remove', closestPlant.id, myLocation)
                        else
                            if stage.interact then
                                interactHelp(closestPlant.stage, _U(stage.label))
                                if IsControlJustPressed(0, 38) then
                                    lastAction = now
                                    TriggerServerEvent('esx_uteknarkCoke:frob', closestPlant.id, myLocation)
                                end
                            else
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        cropstateCoke.octree:searchSphereAsync(here, drawDistance, function(entry)
            if not entry.data.object and not entry.data.deleted then
                local stage = entry.data.stage or 1
                local model = GrowthCoke[stage].model
                ...
                if not HasModelLoaded(model) then
                    Citizen.Trace("Failed to load model for GrowthCoke stage " .. stage ..", but will retry shortly!\n")
                    Citizen.Wait(2500)
                else
                    local offset = GrowthCoke[stage].offset or vector3(0,0,0)
...
RegisterNetEvent('esx_uteknarkCoke:toggle_debug')
AddEventHandler ('esx_uteknarkCoke:toggle_debug', function()
...
RegisterNetEvent('esx_uteknarkCoke:pyromaniac')
AddEventHandler ('esx_uteknarkCoke:pyromaniac',function(location)
...
RegisterNetEvent('esx_uteknarkCoke:groundmat')
AddEventHandler ('esx_uteknarkCoke:groundmat', function()
    local plantable, message, where, normal, material = getPlantingLocation(true)
...
RegisterNetEvent('esx_uteknarkCoke:test_forest')
AddEventHandler ('esx_uteknarkCoke:test_forest',function(count, randomStage)
    ...
    while planted < count do
        ...
        if found then
            local stage = (planted % #GrowthCoke) + 1
            if randomStage then
                stage = math.random(#GrowthCoke)
            end
    ...
    TriggerServerEvent('esx_uteknarkCoke:test_forest', forest)
end)
