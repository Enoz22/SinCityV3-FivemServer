...
local cropstateMethods = {
    plant = function(instance, location, soil, stage, source)
        if onServer then
            ...
            xPlayer = ESX.GetPlayerFromIdentifier(source)-- ajout d'une team
			--print(json.encode(xPlayer))
			--print("Uteknark-cropstate-OrgPlayer : "..xPlayer.org.name)-- ajout d'une team
            end
            MySQL.Async.insert("INSERT INTO `uteknark` (`player`, `x`, `y`, `z`, `soil`, `stage`) VALUES (@player, @x, @y, @z, @soil, @stage);",
            {
                ['@player'] = source,
				--['@org'] = xPlayer.org.name,-- ajout d'une team
                ['@x'] = location.x,
                ['@y'] = location.y,
                ['@z'] = location.z,
                ['@soil'] = soil,
            ...
            function(id)
                instance:import(id, location, stage, os.time(), soil, source) -- ajout d'une team
    ...
    import = function(instance, id, location, stage, time, soil, Playerorg , source)
        local success, object = instance.octree:insert(location, 0.01, {id=id, stage=stage, time=time, soil=soil, team=Playerorg, player=source}) -- ajout d'une team
        if not success then
    ...
    update = function(instance, id, stage, yield)
        ...
        if onServer then
            ...
            MySQL.Async.execute("UPDATE `uteknark` SET `stage` = @stage WHERE `id` = @id;",
            {
                ['@id'] = id,
                ['@stage'] = stage,
            }, function(_)
...
if onServer then
	...
	function HappyHourHarvestFightzone(source)
		--print((tonumber(os.date("%H")) > 22) == (tonumber(os.date("%H")) < 23),(tonumber(os.date("%H")) > 22),(tonumber(os.date("%H")) < 23))

			print(not ((tonumber(os.date("%H")) > 22) == (tonumber(os.date("%H")) < 23)))
			if not ((tonumber(os.date("%H")) > 22) == (tonumber(os.date("%H")) < 23)) then
				TriggerClientEvent('esx:showNotification', source, "La récolte/destruction par les ennemies ne peut se faire qu'à 22h-23h")
				return false
			end
			--else
			--	if (tonumber(os.date("%H")) < 16) then
			--		TriggerClientEvent('esx:showNotification', source, "La plante ne peut plus être planté en dehors des heures d'affluence (16h-22h)")
			--		return false
			--	end
			--end
			return true
    ...
    AddEventHandler ('esx_uteknark:frob', function(plantID, nearLocation)
		...
		--local Owner = false
		--if xPlayer.org.name == "unemployed2" and plant.data.source == xPlayer.identifier then
		--	local Owner = true
		--else
		--	if plant.data.team == xPlayer.org.name then
		--		local Owner = true
		--	end
		--end
		--print(Owner)
		--print((tonumber(os.date("%H")) > 22) == (tonumber(os.date("%H")) < 23),(tonumber(os.date("%H")) > 22),(tonumber(os.date("%H")) < 23))
		--if HappyHourHarvestFightzone(source) then
		--	print("Intime")
		--else
		--	print("Notintime")
		--	if Owner == false then
			--	makeToast(src, _U('interact_text'), "Vous ne pouvez récolté qu'entre 22h et 23h [Fight Hour]")
		--		return
		--	end
		--end
            ...
            if distance <= Config.Distance.Interact then
                ...
                if stageData.interact then
                    if stageData.yield then
                         -- ajout HappyHour + Savoir si le propriétaire est de la meme team
