        ...
        Citizen.Wait(0)
            ...
            for i = 1, #shisha do
                ...
                if dist < 3.5 then
					...
					if not buyad then
						ESX.Game.Utils.DrawText3D(vector3(shishaPos.x, shishaPos.y, shishaPos.z+1), "Appuyez sur [~y~E~w~] pour commencer a fumer.", 0.7)
					end
                    ...
                    if IsControlJustReleased(0, 38) then
						...
						sessionStarted = true
					    endCallback = true
                        ESX.ShowNotification("~g~Vous avez commencer a fumer.", false, false)
						ShishaFunctions()
						...
						Citizen.CreateThread(function()
							while true do
								if sessionStarted == true then
									ESX.Game.Utils.DrawText3D(vector3(shishaPos.x, shishaPos.y, shishaPos.z+0.4), "~c~Appuyez sur ~b~\"H\"~c~ pour vapoter.", 0.7)
									ESX.Game.Utils.DrawText3D(loc, "~c~Appuyez sur ~b~\"F\"~c~ pour arreter de fumer.", 0.7)
								end
								Citizen.Wait(0)
							end
						end)
	...
	local ped = GetPlayerPed(-1)
		...
		if IsControlJustReleased(0, 23) then -- Normal: F
			if endCallback == true then
				...
				ESX.ShowNotification("~r~Vous avez arreter de fumer.", false, false)
				endCallback = false
			end
		end
    end
...
AddEventHandler("only_hooka:delete", function(source)
	...
	for i = 1, #shisha do
		...
		if dist < 3.5 then
			...
			ESX.ShowNotification("~g~Chicha reprise.", false, false)
		else
			Citizen.Wait(500)
		end
	end
