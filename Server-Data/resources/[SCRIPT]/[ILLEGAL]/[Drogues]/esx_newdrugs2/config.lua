...
Config.PoliceBlipShow = false			-- enable or disable blip on map on police notify
...
-- Job Settings:
Config.CooldownTime = 0					    -- Set cooldown time for doing drug jobs in minutes
...
Config.ListOfDrugs = {
	...
	{ drug = 'opium', label = 'Opium', Enabled = true, BuyPrice = 4500, MinReward = 4, MaxReward = 10 },
...
Config.Jobs = {
    { 
		...
		Goons = {
			...
			NPC2 = { x = -222.18724060059, y = 6390.8276367188, z = 31.731483459473, h = 132.96998596192, ped = 'G_M_Y_MexGang_01', animDict = 'rcmme_amanda1', animName = 'stand_loop_cop', weapon = 'WEAPON_DBSHOTGUN', },
			...
			NPC5 = { x = -221.62728881836, y = 6375.7763671875, z = 35.193054199219, h = 36.372509002686, ped = 'G_M_Y_MexGang_01', animDict = 'rcmme_amanda1', animName = 'stand_loop_cop', weapon = 'WEAPON_REVOLVER', }
		}
	},
	{ 
		...
		Goons = {
			...
			NPC3 = { x = -669.90759277344, y = 5796.826171875, z = 17.330947875977, h = 133.18479919434, ped = 'G_M_Y_SalvaBoss_01', animDict = 'amb@world_human_leaning@male@wall@back@legs_crossed@base', animName = 'base', weapon = 'WEAPON_MINISMG' },
...
Config.DrugSaleCooldown = 2						-- Cooldown between each selling in seconds.
Config.SellDrugsBarText = "Parlez a un client"  -- Progress Bar Text for selling drugs
Config.SellDrugsTime = 5						-- time taken to negotiate with NPC in seconds
...
Config.WeedDrug = "joint2g"						-- item name in database
Config.OpiumDrug = "opium4g"						-- item name in database
Config.CallPoliceChance = 3 					-- 2 means 50%, 3 means 33%, 4 means 25% and etc
...
Config.DrugEffects = {
	...
	{ 
		UsableItem 				= "gitanes",						-- item name in database for usable item
		ProgressBarText			= "SMOKING",		-- progress bar text
		UsableTime 				= 5000,							-- Smoking time in MS
		EffectDuration 			= 30,							-- Duration for effect in seconds
		FasterSprint 			= false,							-- Enable or disable faster sprint while on drugs
		SprintValue 			= 1.2,							-- 1.0 is default
		MotionBlur 				= true,							-- Enable or disable motion blur while on drugs
		TimeCycleModifier		= false,							-- Enable or disable time cycle modifer while on drugs
		TimeCycleModifierType	= "spectator5",					-- Set type of time cycle modificer, see full list here: https://pastebin.com/kVPwMemE 
	...
	{ 
		UsableItem 				= "opium4g",					-- item name in database for usable item
		ProgressBarText			= "SMOKING JOINT",				-- progress bar text
		UsableTime 				= 5000,							-- item name in database for usable item
		EffectDuration 			= 30,							-- Duration for effect in seconds
		FasterSprint 			= false,						-- Enable or disable faster sprint while on drugs
...
Config.DrugConversion = {
	{ 
		...
		RewardAmount 			= {a = 5, b = 10},				-- Amount of RewardItem player receives. "a" is without scale and "b" is with scale
		...
		RequiredItemAmount 		= {c = 5, d = 10},				-- Amount of RequiredItem for conversion. "c" is without scale and "d" is with scale
		...
		ProgressBarText			= "Brique de coke en decoupage",   -- progress bar text
		ConversionTime			= 50000,							-- set conversion time in MS.
	},
	{ 
		...
		MaxRewardItemInv		= {e = 44, f = 40},				-- Amount of RewardItem player can hold in inventory. "e" is without scale and "f" is with scale
		RequiredItem 			= "drugbags2",					-- item name in database for required item
		RequiredItemAmount 		= {c = 6, d = 10},				-- Amount of RequiredItem for conversion. "c" is without scale and "d" is with scale
		HighQualityScale		= true,							-- enable/disable scale feature for the drugType
		hqscale					= "hqscale",					-- item name in database for hiqh quality scale item
		ProgressBarText			= "Fiole de Meth en écoulement",		-- progress bar text
		ConversionTime			= 50000,							-- set conversion time in MS.
	},
	{ 
		...
		HighQualityScale		= true,							-- enable/disable scale feature for the drugType
		hqscale					= "hqscale",					-- item name in database for hiqh quality scale item
		ProgressBarText			= "Brique de weed en decoupage",		-- progress bar text
		ConversionTime			= 50000,							-- set conversion time in MS.
	},
	{ 
		...
		HighQualityScale		= true,							-- enable/disable scale feature for the drugType
		hqscale					= "hqscale",					-- item name in database for hiqh quality scale item
		ProgressBarText			= "Sachet de coke en decoupage",			-- progress bar text
	...
	{ 
		...
		HighQualityScale		= true,							-- enable/disable scale feature for the drugType
		hqscale					= "hqscale",					-- item name in database for hiqh quality scale item
		ProgressBarText			= "Fiole de Meth en encoulement",			-- progress bar text
		ConversionTime			= 5000,							-- set conversion time in MS.
	},
	{ 
		...
		HighQualityScale		= true,							-- enable/disable scale feature for the drugType
		hqscale					= "hqscale",					-- item name in database for hiqh quality scale item
		ProgressBarText			= "Sachet de Weed en decoupage",			-- progress bar text
		ConversionTime			= 5000,							-- set conversion time in MS.
	},
	{ 
		...
		hqscale					= "hqscale",					-- item name in database for hiqh quality scale item
		ProgressBarText			= "Pochon de Weed en roulement",			-- progress bar text
		ConversionTime			= 5000,							-- set conversion time in MS.
	},
	{ 
		UsableItem 				= "opium10g",						-- item name in database for usable item
		RewardItem 				= "opium4g",					-- item name in database for required item
		RewardAmount 			= {a = 2, b = 5},							-- Amount of RewardItem player receives
		MaxRewardItemInv		= {e = 44, f = 40},				-- Amount of RewardItem player can hold in inventory. "e" is without scale and "f" is with scale
		RequiredItem 			= "drugbags",					-- item name in database for required item
		RequiredItemAmount 		= {c = 6, d = 10},							-- Amount of RequiredItem for conversion
		HighQualityScale		= true,						-- enable/disable scale feature for the drugType
		hqscale					= "hqscale",					-- item name in database for hiqh quality scale item
		ProgressBarText			= "Sachet d'opium en decoupage",			-- progress bar text
		ConversionTime			= 5000,							-- set conversion time in MS.
	},
	{ 
		UsableItem 				= "plastique",						-- item name in database for usable item
		RewardItem 				= "drugbags",					-- item name in database for required item
		RewardAmount 			= {a = 1, b = 1},							-- Amount of RewardItem player receives
		MaxRewardItemInv		= {e = 44, f = 40},				-- Amount of RewardItem player can hold in inventory. "e" is without scale and "f" is with scale
		RequiredItem 			= "plastique",					-- item name in database for required item
		RequiredItemAmount 		= {c = 5, d = 5},							-- Amount of RequiredItem for conversion
		HighQualityScale		= false,						-- enable/disable scale feature for the drugType
		hqscale					= "hqscale",					-- item name in database for hiqh quality scale item
		ProgressBarText			= "Sachet en decoupage",			-- progress bar text
		ConversionTime			= 5000,							-- set conversion time in MS.
	},
	{ 
		UsableItem 				= "opium4g",						-- item name in database for usable item
		RewardItem 				= "opium1g",					-- item name in database for required item
		RewardAmount 			= {a = 2, b = 3},							-- Amount of RewardItem player receives
		MaxRewardItemInv		= {e = 44, f = 40},				-- Amount of RewardItem player can hold in inventory. "e" is without scale and "f" is with scale
		RequiredItem 			= "chicha",					-- item name in database for required item
		RequiredItemAmount 		= {c = 1, d = 1},							-- Amount of RequiredItem for conversion
		HighQualityScale		= true,						-- enable/disable scale feature for the drugType
		hqscale					= "hqscale",					-- item name in database for hiqh quality scale item
		ProgressBarText			= "Pochon d'opium en transformation",			-- progress bar text
		ConversionTime			= 5000,							-- set conversion time in MS.
	}
}
