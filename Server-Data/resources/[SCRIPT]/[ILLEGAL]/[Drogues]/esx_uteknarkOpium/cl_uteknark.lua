...
function interactHelp(stage,action)
    ...
    AddTextComponentInteger(#GrowthOpium)
...
function serverlog(...)
    TriggerServerEvent('esx_uteknarkOpium:log',...)
end

RegisterNetEvent('esx_uteknarkOpium:make_toast')
AddEventHandler ('esx_uteknarkOpium:make_toast', function(subject,message)
        ...
        if Config.Soil[material] then
            ...
            if flatEnough(surfaceNormal) then
                ...
                if plantDistance <= Config.Distance.Interact then
                    local hits = cropstateOpium.octree:searchSphere(hitLocation, Config.Distance.Space)
                    if #hits > 0 then
...
RegisterNetEvent('esx_uteknarkOpium:do')
AddEventHandler ('esx_uteknarkOpium:do', function(scenarioName, location)
...
RegisterNetEvent('esx_uteknarkOpium:attempt_plant')
AddEventHandler ('esx_uteknarkOpium:attempt_plant', function()
    local plantable, message, location, _, soil = getPlantingLocation()
    if plantable then
        TriggerServerEvent('esx_uteknarkOpium:success_plant', location, soil)
        ...
        if #activePlants > 0 then
            ...
            if closestDistance and not IsPedInAnyVehicle(playerPed) then
                ...
                if closestDistance <= Config.Distance.Interact then
                    local stage = GrowthOpium[closestPlant.stage]
                    ...
                    if now >= lastAction + Config.ActionTime then
                        if IsDisabledControlJustPressed(0, 44) then
                            ...
                            TriggerServerEvent('esx_uteknarkOpium:remove', closestPlant.id, myLocation)
                        else
                            if stage.interact then
                                interactHelp(closestPlant.stage, _U(stage.label))
                                if IsControlJustPressed(0, 38) then
                                    lastAction = now
                                    TriggerServerEvent('esx_uteknarkOpium:frob', closestPlant.id, myLocation)
                                end
                            else
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        cropstateOpium.octree:searchSphereAsync(here, drawDistance, function(entry)
            if not entry.data.object and not entry.data.deleted then
                local stage = entry.data.stage or 1
                local model = GrowthOpium[stage].model
                ...
                if not HasModelLoaded(model) then
                    Citizen.Trace("Failed to load model for GrowthOpium stage " .. stage ..", but will retry shortly!\n")
                    Citizen.Wait(2500)
                else
                    local offset = GrowthOpium[stage].offset or vector3(0,0,0)
...
RegisterNetEvent('esx_uteknarkOpium:toggle_debug')
AddEventHandler ('esx_uteknarkOpium:toggle_debug', function()
...
RegisterNetEvent('esx_uteknarkOpium:pyromaniac')
AddEventHandler ('esx_uteknarkOpium:pyromaniac',function(location)
...
RegisterNetEvent('esx_uteknarkOpium:groundmat')
AddEventHandler ('esx_uteknarkOpium:groundmat', function()
    local plantable, message, where, normal, material = getPlantingLocation(true)
...
AddEventHandler ('esx_uteknarkOpium:test_forest',function(count, randomStage)
    ...
    while planted < count do
        ...
        if found then
            local stage = (planted % #GrowthOpium) + 1
            if randomStage then
                stage = math.random(#GrowthOpium)
            end
    ...
    TriggerServerEvent('esx_uteknarkOpium:test_forest', forest)
end)
