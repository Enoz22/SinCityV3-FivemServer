    ...
    while true do
        ...
        if payAmount > 0 then
            for shop = 1, #Config.Locations do
                ...
                if dist <= 20.0 then
                    if dist >= 12.0 then
                        ESX.ShowNotification("Vous avez quitté le magasin, votre panier est maintenant vide !")
                        payAmount = 0
                        Basket = {}
                    end
                end
...
OpenAction = function(action, shelf, text)
    if action["value"] == "checkout" then
        if payAmount > 0 and #Basket then
            ...
            ESX.ShowNotification("Vous n’avez rien dans votre panier !")
        end
    else
...
CashRegister = function(titel)
        local elements = {
            {label = '<span style="color:lightgreen; border-bottom: 1px solid lightgreen;">Confirmer</span>', value = "yes"},
            {label = '<span style="color:red;">Vider le panier</span>', value ="empty"},
        ...
        ESX.UI.Menu.Open(
            ...
            function(data, menu)
                if data.current.value == "empty" then
                    Basket = {};
                    menu.close();
                    ESX.ShowNotification('Panier vidé, merci de votre visite et à bientôt.');
                end
                        ...
                        function(data2, menu2)
                            ESX.TriggerServerCallback('99kr-shops:CheckMoney', function(hasMoney)
                                if hasMoney then
                                    ...
                                    ESX.ShowNotification("Tu n'a pas assez d'argent!")
                                end
...
ShelfMenu = function(titel, shelf)
    ...
    ESX.UI.Menu.Open(
        ...
        function(data, menu)
            ...
            ESX.ShowNotification("Mettre " .. data.current.value .. " " .. data.current["realLabel"] .. " dans le panier")          
        end,
    function(data, menu)
        menu.close()
    end)
        ...
        ESX.UI.Menu.Open(
            ...
            function(data, menu)
                if data.current.value == 'empty' then
                    ...
                    payAmount = 0
                    menu.close()
                    ESX.ShowNotification("Tout retirer de votre panier.")
                end
