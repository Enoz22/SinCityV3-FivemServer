...
AddEventHandler("niso-bank:checkATM", function()
    if PlayerNearATM() then
        ...
        TriggerEvent("notification", "Il n'y a pas d'ATM", 2)
    end
end)
        ...
        if ESX ~= nil then
            ...
            for k,v in pairs(Config.Banks) do
                if (#(v.Location - pos) < 1) then
                    drawTxt(v.Location.x, v.Location.y, v.Location.z, "[E] Utiliser la Banque..")
                ...
                elseif (#(v.Location - pos) < 4.5) then
                    drawTxt(v.Location.x, v.Location.y, v.Location.z, "Utiliser la Banque")
                end
            end
        end
    end
