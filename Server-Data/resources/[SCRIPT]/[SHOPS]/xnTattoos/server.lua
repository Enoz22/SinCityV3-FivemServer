...
ESX.RegisterServerCallback('SmallTattoos:PurchaseTattoo', function(source, cb, tattooList, price, tattoo, tattooName)
	...
	if xPlayer.getMoney() >= price then
		...
		TriggerClientEvent('esx:showNotification', source, "Vous avez acheté le tatouage ~y~" .. tattooName .. "~s~ pour ~g~$" .. price .."~s~.")
		cb(true)
	else
		TriggerClientEvent('esx:showNotification', source, "Vous n'avez pas assez d'argent pour acheter ce tatouage.")
		cb(false)
	end
end)
