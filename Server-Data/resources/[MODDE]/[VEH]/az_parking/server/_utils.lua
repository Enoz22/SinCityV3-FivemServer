...
_Utils.SendServerNotification = function(target, message, type)
	TriggerClientEvent("pNotify:SendNotification", target, {
		text = message,
		type = type,
		queue = "mg_realparking",
	})
end

_Utils.SaveCarInDatabase = function(xPlayer, vehicle, plate, garageType, callback, onSaved)
	MySQL.Async.fetchAll("SELECT owner,plate,`stored` FROM owned_vehicles WHERE `owner` = @identifier AND `plate` = @plate", {
		...
		elseif type(rs) == 'table' and #rs > 0 and rs[1] ~= nil and not rs[1].stored then
			MySQL.Async.execute("UPDATE owned_vehicles SET `stored` = 1, `location` = @location, `garage_time` = @time, `garage_name` = @garage_name, `vehicle` = @vehicle, `garage_type` = @garage_type WHERE `plate` = @plate AND `owner` = @identifier",
			{
