...
RegisterServerEvent('mg_realparking:addGlobalVehicle')
AddEventHandler('mg_realparking:addGlobalVehicle', function(vehicle, garageName)
	TriggerClientEvent("mg_realparking:addVehicle", -1, vehicle, garageName)
end)

RegisterServerEvent('mg_realparking:removeGlobalVehicle')
AddEventHandler('mg_realparking:removeGlobalVehicle', function(source, vehicle, garageName)
	MySQL.Async.execute('UPDATE owned_vehicles SET `stored` = 0, `location` = NULL, `garage_name` = NULL, `garage_time` = NULL WHERE `plate` = @plate', {
		["@plate"]      = vehicle.plate
	})
	TriggerClientEvent("mg_realparking:deleteVehicle", -1, vehicle, garageName)
	RemoveVehicleFromZone(vehicle)
end)

RegisterServerEvent('mg_realparking:impoundVehicle')
AddEventHandler('mg_realparking:impoundVehicle', function(cb, vehicleProps, garageName)
	...
	'UPDATE owned_vehicles SET vehicle=@props, pound=1, location=NULL, garage_time=NULL, garage_name=NULL WHERE plate=@plate',
	{
		...
		if rs and (not rs.errorCount) then
			...
			TriggerClientEvent("mg_realparking:deleteVehicle", -1, vehicleData, garageName)
...
RegisterServerEvent('mg_realparking:getVehicleInfo')
AddEventHandler('mg_realparking:getVehicleInfo', function(cb, plate, fromCallback, player)
	local xPlayer = nil
	local _source = source
...
ESX.RegisterServerCallback("mg_realparking:getZoneVehicles", function(source, cb, garageName)
...
ESX.RegisterServerCallback("mg_realparking:getCarParkingPrice", function(source, cb, plate)
	if Config.Debug then
...
ESX.RegisterServerCallback("mg_realparking:saveVehicle", function(source, cb, vehicle)
...
ESX.RegisterServerCallback("mg_realparking:driveVehicle", function(source, callback, vehicle, garageName)
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	MySQL.Async.fetchAll("SELECT owner,plate,type,vehicle,garage_name,garage_time,location,job FROM owned_vehicles WHERE `owner` = @identifier AND `plate` = @plate AND `stored` = 1", {
		...
		if type(rs) == 'table' and #rs > 0 and rs[1] ~= nil then
			...
			if playerMoney >= fee then
				...
				TriggerClientEvent("mg_realparking:deleteVehicle", -1, vehicleData, rs[1].garage_name)
				RemoveVehicleFromZone(vehicleData)
...
ESX.RegisterServerCallback('mg_realparking:payMoney', function(source, cb, money, society)
	local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
...
ESX.RegisterServerCallback("mg_realparking:getVehicleCallback", function(source, callback, plate)
