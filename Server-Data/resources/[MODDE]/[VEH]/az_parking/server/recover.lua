...
ESX.RegisterServerCallback("mg_realparking:getOutFromRecover", function(source, callback, vehicle)
...
ESX.RegisterServerCallback('mg_realparking:getNotStoredCars', function(source, cb)
    local _source = source
    xPlayer = ESX.GetPlayerFromId(_source)
    identifier = xPlayer.identifier

    MySQL.Async.fetchAll(
    'SELECT owned_vehicles.*, vehicle_model_prices.price FROM owned_vehicles LEFT JOIN vehicle_model_prices ON owned_vehicles.model = vehicle_model_prices.model WHERE owner=@identifier AND (`stored`=FALSE AND pound=FALSE) AND (job IS NULL OR job = @job)',
    {
