...
function ShowWarehouseMenu()
	ESX.TriggerServerCallback('mg_realparking:getImpoundedVehicles', function(vehicles)
...
function OpenImpoundGUI()
	...
	if vehicle == nil or (#(GetEntityCoords(vehicle) - playerCoords) > 4.0) then
		...
		ESX.TriggerServerCallback('mg_realparking:getVehicleCallback', function(vehicleData)
...
RegisterNUICallback('unimpound', function(plate, cb)
	if currentWarehouse then
		...
		TriggerServerEvent('mg_realparking:unimpoundVehicle', plate, currentWarehouse, spawnNumber);
	else
...
RegisterNUICallback('impound', function(data, cb)
	...
	TriggerServerEvent('mg_realparking:generateImpound', data, vehicleProps, vehicle)
	cb(true)
	CloseGUI()
end)
...
RegisterNetEvent('mg_realparking:spawnUnimpoundedVehicle')
AddEventHandler('mg_realparking:spawnUnimpoundedVehicle', function (data, zone, spawnNumber)
...
-- Open GUI Handler
RegisterNetEvent('mg_realparking:openImpoundGUI')
AddEventHandler('mg_realparking:openImpoundGUI', function ()
	OpenImpoundGUI()
end)

-- Ensure entity removed
RegisterNetEvent('mg_realparking:deleteEntity')
AddEventHandler('mg_realparking:deleteEntity', function (entity, plate)
