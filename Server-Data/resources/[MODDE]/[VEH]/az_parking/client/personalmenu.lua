...
function ChangeVehicleName(vehicle)
	...
	function(dialog, menu)
		...
		if string.len(dialog.value) >= 1 then
			TriggerServerEvent('mg_realparking:renamevehicle',  vehicle.plate, dialog.value)
...
function GenerateVehicleAndSendToUser(vehData, price, society)
	...
	InValidation = true
	ESX.TriggerServerCallback('mg_realparking:payMoney', function(hasEnoughMoney, left)
		InValidation = false
		if hasEnoughMoney then
			...
			Citizen.CreateThread(function()
				...
				TriggerServerEvent("mg_realparking:removeGlobalVehicle", -1, vehicleData, vehData.garage_name)
...
function OpenStoredVehicleMenu(vehicle)
	ESX.TriggerServerCallback("mg_realparking:getCarGaragePrice", function(price, garageFee)
		ESX.UI.Menu.CloseAll()
		local elements = {}
...
function OpenCarMenu()
	...
	ESX.UI.Menu.CloseAll()
	ColdDownMenu = 5
	ESX.TriggerServerCallback('mg_realparking:getAllCars', function(vehicles)
		local elements = {}
...
RegisterNetEvent("mg_realparking:openPersonalMenu")
AddEventHandler("mg_realparking:openPersonalMenu", function()
	OpenCarMenu()
end)
